-- =============================================================================
-- Payhuk prod — authHero public + rétention platform_visitor_events
-- Coller tout ce fichier dans Supabase → SQL Editor → Run
-- =============================================================================

BEGIN;

-- 1) RPC personnalisation publique (authHero + landing images)
...
-- =============================================================================
-- 4) Purge immédiate (libère Disk IO maintenant)
-- =============================================================================
SELECT public.cleanup_platform_visitor_events(30) AS rows_deleted;

-- =============================================================================
-- 5) Vérifications
-- =============================================================================
SELECT count(*) AS remaining_events,
       pg_size_pretty(pg_total_relation_size('public.platform_visitor_events')) AS table_size
FROM public.platform_visitor_events;

SELECT get_public_platform_customization() -> 'media' -> 'images' ->> 'authHero' AS public_auth_hero;

SELECT jobid, jobname, schedule
FROM cron.job
WHERE jobname = 'cleanup-platform-visitor-events';

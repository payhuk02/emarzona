...
DROP TRIGGER IF EXISTS update_course_certificate_templates_updated_at ON public.course_certificate_templates;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stores') THEN
    IF NOT EXISTS (
      ...
      EXECUTE '
        DROP POLICY IF EXISTS "Store owners can manage certificate templates" ON public.course_certificate_templates;
CREATE POLICY "Store owners can manage certificate templates"
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'course_enrollments') THEN
    IF NOT EXISTS (
      ...
      EXECUTE '
        DROP POLICY IF EXISTS "Users can view active templates" ON public.course_certificate_templates;

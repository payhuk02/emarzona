...
CREATE TABLE IF NOT EXISTS public.user_pixels (
...
DROP POLICY IF EXISTS "Users can view their own pixels" ON public.user_pixels;
...
DROP POLICY IF EXISTS "Users can create their own pixels" ON public.user_pixels;
...
DROP POLICY IF EXISTS "Users can update their own pixels" ON public.user_pixels;
...
DROP POLICY IF EXISTS "Users can delete their own pixels" ON public.user_pixels;
...
DROP POLICY IF EXISTS "Users can create their own pixel events" ON public.pixel_events;
...
DROP TRIGGER IF EXISTS update_user_pixels_updated_at ON public.user_pixels;
...
CREATE INDEX IF NOT EXISTS idx_user_pixels_user_id ON public.user_pixels(user_id);
CREATE INDEX IF NOT EXISTS idx_user_pixels_active ON public.user_pixels(user_id, is_active);

...
DROP POLICY IF EXISTS "Order items viewable by store owners" ON public.order_items;
...
DROP POLICY IF EXISTS "Order items insertable by store owners" ON public.order_items;
...
DROP POLICY IF EXISTS "Store owners can insert payments" ON public.payments;
CREATE POLICY "Store owners can insert payments" ON public.payments FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public.stores WHERE id = store_id AND user_id = auth.uid())
);
DROP POLICY IF EXISTS "Store owners can update payments" ON public.payments;
...
DROP POLICY IF EXISTS "Store owners can insert transactions" ON public.transactions;
CREATE POLICY "Store owners can insert transactions" ON public.transactions FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public.stores WHERE id = store_id AND user_id = auth.uid())
);
DROP POLICY IF EXISTS "Store owners can update transactions" ON public.transactions;

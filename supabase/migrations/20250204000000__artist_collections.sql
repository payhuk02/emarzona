...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    EXECUTE '
      DROP POLICY IF EXISTS "Public can view public collections" ON public.artist_collections;
CREATE POLICY "Public can view public collections"
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stores') THEN
    IF NOT EXISTS (
      ...
      EXECUTE '
        DROP POLICY IF EXISTS "Store owners can manage their collections" ON public.artist_collections;
CREATE POLICY "Store owners can manage their collections"
  ...
  IF NOT EXISTS (
    ...
    EXECUTE '
      DROP POLICY IF EXISTS "Public can view collection items for public collections" ON public.artist_collection_items;

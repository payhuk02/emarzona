...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    EXECUTE '
      DROP POLICY IF EXISTS "Users can view their own progression snapshots" ON public.course_progression_snapshots;
CREATE POLICY "Users can view their own progression snapshots"
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stores')
    ...
    IF NOT EXISTS (
      ...
      EXECUTE '
        DROP POLICY IF EXISTS "Store owners can view course progression analytics" ON public.course_progression_analytics;
CREATE POLICY "Store owners can view course progression analytics"
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'stores')
    ...
    IF NOT EXISTS (
      ...
      EXECUTE '
        DROP POLICY IF EXISTS "Store owners can view lesson analytics" ON public.course_lesson_analytics;

-- =========================================================
-- Migration : Système de Cohorts Avancé pour Cours
...
DO $$
BEGIN
  -- Ajouter store_id si elle n'existe pas
  IF NOT EXISTS (
  ...
  -- Ajouter cohort_name si elle n'existe pas (peut être 'name' dans l'ancienne version)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'course_cohorts' 
    AND column_name = 'cohort_name'
  ) THEN
    -- Vérifier si 'name' existe et la renommer
    IF EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_schema = 'public' 
      AND table_name = 'course_cohorts' 
      ...
      ALTER TABLE public.course_cohorts 
      RENAME COLUMN name TO cohort_name;
    ELSE
      ALTER TABLE public.course_cohorts 
  ...
  -- Ajouter cohort_slug si elle n'existe pas
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'course_cohorts' 
    AND column_name = 'cohort_slug'
  ) THEN
    ALTER TABLE public.course_cohorts 
    ADD COLUMN cohort_slug TEXT;
  END IF;
END $$;
...
CREATE OR REPLACE FUNCTION calculate_cohort_analytics(p_cohort_id UUID, p_date DATE DEFAULT CURRENT_DATE)
RETURNS UUID AS $$
DECLARE
  v_analytics_id UUID;
...
BEGIN
  ...
  IF EXISTS (
    ...
    IF NOT EXISTS (
      ...
      DROP TRIGGER IF EXISTS update_cohorts_updated_at ON public.course_cohorts;
CREATE TRIGGER update_cohorts_updated_at
  ...
  IF EXISTS (
    ...
    IF NOT EXISTS (
      ...
      DROP TRIGGER IF EXISTS update_enrollments_updated_at ON public.cohort_enrollments;
CREATE TRIGGER update_enrollments_updated_at
    ...
    IF NOT EXISTS (
      ...
      DROP TRIGGER IF EXISTS update_cohort_student_count_trigger ON public.cohort_enrollments;
...
DO $$
BEGIN
  -- Vérifier que la table existe ET que les colonnes nécessaires existent
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema = 'public' 
    AND table_name = 'course_cohorts'
  ...
  -- Vérifier que la table cohort_analytics existe ET que course_cohorts a la colonne store_id
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema = 'public' 

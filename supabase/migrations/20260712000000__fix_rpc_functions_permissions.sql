-- Fix RPC functions permissions and ensure they exist
-- Date: 2026-07-12

BEGIN;

-- Ensure is_store_member function exists and has proper permissions
...
-- Ensure get_store_dashboard_stats_aggregated function exists
...
BEGIN
  ...
  -- Produits (inventaire boutique, hors filtre période)
  ...
  -- Commandes période courante + période précédente (même durée)
  ...
  -- Performance par type (commandes complétées dans la période)
  ...
  -- Top produits (revenus période, commandes complétées)
  SELECT COALESCE(
  ...
  -- Commandes récentes dans la période
  SELECT COALESCE(
...
COMMENT ON FUNCTION public.get_store_dashboard_stats_aggregated(UUID, TIMESTAMPTZ, TIMESTAMPTZ, TEXT) IS
  'Statistiques dashboard agrégées en une requête (période configurable, contrôle is_store_member).';

-- Ensure get_top_selling_products function exists
CREATE OR REPLACE FUNCTION public.get_top_selling_products(store_uuid UUID, limit_count INTEGER DEFAULT 10)
RETURNS TABLE (
  id UUID,
  name TEXT,
  price DECIMAL,
...
GRANT EXECUTE ON FUNCTION public.get_top_selling_products(UUID, INTEGER) TO authenticated;

COMMENT ON FUNCTION public.get_top_selling_products(UUID, INTEGER) IS

...
DROP TRIGGER IF EXISTS cookie_preferences_updated_at ON public.cookie_preferences;
...
DROP POLICY IF EXISTS "Users can view own cookie preferences" ON public.cookie_preferences;
CREATE POLICY "Users can view own cookie preferences"
  ON public.cookie_preferences FOR SELECT
  USING (auth.uid() = user_id OR user_id IS NULL);

DROP POLICY IF EXISTS "Users can insert own cookie preferences" ON public.cookie_preferences;
CREATE POLICY "Users can insert own cookie preferences"
  ON public.cookie_preferences FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own cookie preferences" ON public.cookie_preferences;
...
DROP POLICY IF EXISTS "Admins can update GDPR requests" ON public.gdpr_requests;

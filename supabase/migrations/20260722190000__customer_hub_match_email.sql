-- Fix hub acheteur : matcher aussi customers.email (achats invité → compte lié)
-- Aligné resolveBuyerCustomerIds (user_id + email + legacy uid).
...
COMMENT ON FUNCTION public.get_customer_hub_summary(INT, BOOLEAN) IS
  'Hub acheteur unifié — commandes via customers.user_id + email + legacy uid.';

-- ============================================================
-- P1.1: Server-side price for project service orders
-- Recalculate from delivery package + gig extras (ignore client quoted_total)
-- ============================================================

-- Remove legacy 11-arg overload (pre-booking_id) to avoid ambiguous RPC resolution
DROP FUNCTION IF EXISTS public.create_public_service_order(
  uuid, uuid, text, text, text, jsonb, uuid, numeric, text, text, boolean
);
...
COMMENT ON FUNCTION public.create_public_service_order(
  ...
  'Create service order. Appointment: catalog price. Project: server sum of delivery_tier package + active gig extras.';

-- Rank ALL active sponsored products before organic (remove max-3 / 1-per-store feed slots).
-- Badge is_sponsored remains any live active campaign; feed_sponsored now matches that set.
...
COMMENT ON FUNCTION public.get_marketplace_products_filtered IS
  'Marketplace feed: all live sponsored products rank before organic; is_sponsored = any active campaign.';

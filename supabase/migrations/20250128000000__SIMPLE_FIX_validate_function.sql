-- =====================================================
-- SOLUTION SIMPLE : Créer validate_unified_promotion
-- Date: 28 Janvier 2025
-- Description: Script le plus simple possible pour créer la fonction
-- =====================================================

-- ÉTAPE 1: Supprimer l'ancienne fonction si elle existe (toutes les versions)
DO $$
BEGIN
    DROP FUNCTION IF EXISTS public.validate_unified_promotion CASCADE;
END $$;
...
BEGIN
  ...
  IF COALESCE(v_promotion.customer_eligibility, 'all') = 'new_customers' AND NOT p_is_first_order THEN
    RETURN jsonb_build_object(
      'valid', false,
  ...
  IF COALESCE(v_promotion.customer_eligibility, 'all') = 'existing_customers' AND p_is_first_order THEN
    RETURN jsonb_build_object(
      'valid', false,
  ...
  IF COALESCE(v_promotion.first_time_buyers_only, FALSE) AND NOT p_is_first_order THEN
    RETURN jsonb_build_object(
      'valid', false,
  ...
  IF p_store_id IS NOT NULL 
     AND v_promotion.applicable_store_ids IS NOT NULL 
  ...
  IF p_product_ids IS NOT NULL AND array_length(p_product_ids, 1) > 0 THEN
    IF v_promotion.applies_to = 'specific_products' 
       AND v_promotion.product_ids IS NOT NULL 
       AND array_length(v_promotion.product_ids, 1) > 0 THEN
      v_applies := (p_product_ids && v_promotion.product_ids);
    ELSIF v_promotion.applies_to = 'categories' 
          AND v_promotion.category_ids IS NOT NULL 
          AND array_length(v_promotion.category_ids, 1) > 0 THEN
      ...
      END IF;
    ELSIF v_promotion.applies_to = 'collections' 
          AND v_promotion.collection_ids IS NOT NULL 
          AND array_length(v_promotion.collection_ids, 1) > 0 THEN
  ...
  IF v_promotion.discount_type = 'percentage' THEN
    ...
    IF v_promotion.max_discount_amount IS NOT NULL 
       AND v_discount_amount > v_promotion.max_discount_amount THEN
  ...
  RETURN jsonb_build_object(
    ...
    'order_total_after', GREATEST(0, p_order_amount - v_discount_amount)
  );
END;
$$;
...
DO $$
BEGIN
  RAISE NOTICE '✅ Fonction validate_unified_promotion créée avec succès!';
END $$;

...
-- ============================================================================
-- 1. INDEXES SUPPLÉMENTAIRES POUR PERFORMANCES
-- ============================================================================

-- Indexes pour digital_products (si pas déjà présents)
CREATE INDEX IF NOT EXISTS idx_digital_products_created_at 
  ON public.digital_products(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_digital_products_updated_at 
  ON public.digital_products(updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_digital_products_version 
  ON public.digital_products(version);
CREATE INDEX IF NOT EXISTS idx_digital_products_encryption 
  ON public.digital_products(encryption_enabled);

-- Indexes pour digital_product_downloads
CREATE INDEX IF NOT EXISTS idx_downloads_download_date 
  ON public.digital_product_downloads(download_date DESC);
CREATE INDEX IF NOT EXISTS idx_downloads_product_user 
  ON public.digital_product_downloads(digital_product_id, user_id);
CREATE INDEX IF NOT EXISTS idx_downloads_session 
  ON public.digital_product_downloads(session_id);

-- Indexes pour digital_licenses
CREATE INDEX IF NOT EXISTS idx_licenses_customer_email 
  ON public.digital_licenses(customer_email);
CREATE INDEX IF NOT EXISTS idx_licenses_issued_at 
  ON public.digital_licenses(issued_at DESC);
CREATE INDEX IF NOT EXISTS idx_licenses_activated_at 
  ON public.digital_licenses(activated_at DESC);

-- ============================================================================
-- 2. VUES UTILES POUR LES DASHBOARDS
-- ============================================================================

-- Vue: Statistiques par produit digital
...
'Statistiques agrégées pour chaque produit digital (downloads, licenses, activations)';

-- Vue: Téléchargements récents avec détails
...
-- Vue: Licenses actives avec détails
...
-- Fonction: Obtenir le nombre de téléchargements restants pour un utilisateur
...
BEGIN
  -- Récupérer la limite de téléchargements
...
'Retourne le nombre de téléchargements restants pour un utilisateur (-1 si illimité)';

-- Fonction: Vérifier si un utilisateur a accès à un produit digital
...
-- Fonction: Obtenir les statistiques de téléchargement pour un produit
...
'Retourne des analytics détaillées des téléchargements pour un produit sur N jours';

-- Fonction: Mettre à jour les statistiques d'un produit digital
...
-- Fonction: Expirer les licenses automatiquement
...
-- Trigger: Mettre à jour les stats après un téléchargement
...
-- ============================================================================
-- 5. TÂCHES PLANIFIÉES (à configurer dans pg_cron si disponible)
-- ============================================================================

/*
-- Expirer les licenses tous les jours à minuit
...
COMMENT ON FUNCTION expire_digital_licenses() IS 
'À exécuter quotidiennement via pg_cron ou un job externe';

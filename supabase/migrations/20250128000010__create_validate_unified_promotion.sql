-- =====================================================
-- CRÉATION : Fonction validate_unified_promotion
-- Date: 28 Janvier 2025
-- Description: Script autonome pour créer la fonction de validation unifiée
-- =====================================================

-- Supprimer toutes les versions existantes de la fonction (au cas où)
DROP FUNCTION IF EXISTS validate_unified_promotion() CASCADE;
DROP FUNCTION IF EXISTS validate_unified_promotion(TEXT) CASCADE;
DROP FUNCTION IF EXISTS validate_unified_promotion(TEXT, UUID) CASCADE;
DROP FUNCTION IF EXISTS validate_unified_promotion(TEXT, UUID, UUID[]) CASCADE;
DROP FUNCTION IF EXISTS validate_unified_promotion(TEXT, UUID, UUID[], UUID[]) CASCADE;
DROP FUNCTION IF EXISTS validate_unified_promotion(TEXT, UUID, UUID[], UUID[], UUID[]) CASCADE;
DROP FUNCTION IF EXISTS validate_unified_promotion(TEXT, UUID, UUID[], UUID[], UUID[], NUMERIC) CASCADE;
DROP FUNCTION IF EXISTS validate_unified_promotion(TEXT, UUID, UUID[], UUID[], UUID[], NUMERIC, UUID) CASCADE;
DROP FUNCTION IF EXISTS validate_unified_promotion(TEXT, UUID, UUID[], UUID[], UUID[], NUMERIC, UUID, BOOLEAN) CASCADE;
...
-- =====================================================
-- AJOUTER LE COMMENTAIRE (avec la signature complète)
-- =====================================================
...
BEGIN
  ...
  ELSE
    RAISE EXCEPTION '❌ Erreur: La fonction validate_unified_promotion n''a pas été créée';
  END IF;
END $$;

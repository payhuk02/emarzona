...
DROP POLICY IF EXISTS "Admins can insert referral commissions" ON public.referral_commissions;
CREATE POLICY "Admins can insert referral commissions" ON public.referral_commissions
FOR INSERT WITH CHECK (has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins can update referral commissions" ON public.referral_commissions;
CREATE POLICY "Admins can update referral commissions" ON public.referral_commissions
FOR UPDATE USING (has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS "Admins can delete referral commissions" ON public.referral_commissions;

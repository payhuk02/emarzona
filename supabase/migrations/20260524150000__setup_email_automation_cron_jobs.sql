-- Cron pg_net : séquences email + paniers abandonnés (x-cron-secret / CRON_SECRET)

CREATE EXTENSION IF NOT EXISTS pg_net;
CREATE EXTENSION IF NOT EXISTS pg_cron;

-- Séquences : toutes les 15 minutes
...
-- Paniers abandonnés : toutes les heures
...
REVOKE ALL ON FUNCTION public.setup_email_sequences_cron_job(text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.setup_abandoned_cart_recovery_cron_job(text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.setup_email_sequences_cron_job(text, text) TO service_role;
GRANT EXECUTE ON FUNCTION public.setup_abandoned_cart_recovery_cron_job(text, text) TO service_role;

COMMENT ON FUNCTION public.setup_email_sequences_cron_job IS
  'Cron pg_cron → process-email-sequences (toutes les 15 min, header x-cron-secret)';
COMMENT ON FUNCTION public.setup_abandoned_cart_recovery_cron_job IS
  'Cron pg_cron → abandoned-cart-recovery (toutes les heures, header x-cron-secret)';

-- Frais plateforme checkout : 2 % + 100 FCFA (XOF)
-- Appliqué sur create_public_physical_order (digital/course/artist via apply-public-order-rpcs-prod.sql)
...
-- Patch physical RPC if present
DO $$
BEGIN
  IF to_regprocedure('public.create_public_physical_order(uuid,uuid,text,text,text,integer,uuid,text,jsonb,text,boolean)') IS NULL THEN
    RAISE NOTICE 'create_public_physical_order missing — skip physical fee patch';
    RETURN;
  END IF;
END;
$$;

-- Recreate physical order with platform fee (body aligned to 20260706180000 + fee)
...
BEGIN
  ...
  v_total_price := public.apply_checkout_platform_fee(v_subtotal, v_currency);
  v_platform_fee := GREATEST(0, v_total_price - v_subtotal);

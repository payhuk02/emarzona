-- Fix regression: 20260724170000 reintroduced customers.user_id (column does not exist).
-- That aborted process_payment_webhook_atomic on digital paid → orders stayed pending.
...
BEGIN
  ...
  -- customers has no user_id column — resolve auth user via metadata / email
  SELECT
    c.email,
    COALESCE(NULLIF(TRIM(c.name), ''), NULLIF(TRIM(c.full_name), ''), 'Client')
  ...
  LOOP
    IF item.license_id IS NOT NULL THEN
      ...
      SET
        ...
        customer_email = COALESCE(customer_email, v_customer_email),
        updated_at = now()
...
COMMENT ON FUNCTION public.fulfill_digital_order_items_on_paid IS
  'Active/crée licences digitales au paiement ; résout user via metadata/email (pas customers.user_id).';

-- Hotfix prod: products.stock vs stock_quantity (42703 on dashboard RPC)
-- Align schema + RPC with client fallback (useDashboardStats uses products.stock)
...
-- Prod reviews schema lacks moderation columns (see 20260528170000 harden_stores_public_views)
...
-- PostgREST ambiguity: get_unread_count() vs get_unread_count(uuid)
...
BEGIN
  ...
  IF NOT public.is_store_member(store_uuid, v_uid) THEN
    RAISE EXCEPTION 'Access denied to store top products' USING ERRCODE = '42501';
  END IF;
  ...
  SELECT
    ...
    p.image_url,
    p.product_type,
    COALESCE(SUM(oi.total_price), 0)::DECIMAL AS revenue,
    COALESCE(SUM(oi.quantity), 0)::INTEGER AS quantity,

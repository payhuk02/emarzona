-- Expand reserved store slugs / subdomains (sensitive + protected names).
-- Keep aligned with src/lib/store/reserved-store-slugs.ts
...
COMMENT ON FUNCTION public.is_slug_reserved IS
  'Slugs boutique réservés / sensibles (aligné reserved-store-slugs.ts).';
COMMENT ON FUNCTION public.is_subdomain_reserved IS
  'Délègue à is_slug_reserved pour une liste unique.';
COMMENT ON FUNCTION public.is_store_slug_available IS
  'Disponibilité slug boutique : non réservé + unique (insensible à la casse).';

...
-- Recreate views as security definer (default) to bypass base table RLS
-- This is the correct pattern: base table restricted, view exposes safe columns only
DROP VIEW IF EXISTS public.stores_public;
DROP VIEW IF EXISTS public.reviews_public;

-- Security definer view (no security_invoker=on) so it bypasses stores RLS
...
-- Security definer view for reviews (excludes user_id)

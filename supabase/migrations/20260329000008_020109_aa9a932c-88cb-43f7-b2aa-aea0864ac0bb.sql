...
DROP POLICY IF EXISTS "Authenticated users can upload digital files" ON storage.objects;
CREATE POLICY "Authenticated users can upload digital files"
  ON storage.objects FOR INSERT
  TO authenticated
  WITH CHECK (bucket_id = 'digital-files');

DROP POLICY IF EXISTS "Users can read own digital files" ON storage.objects;
CREATE POLICY "Users can read own digital files"
  ON storage.objects FOR SELECT
  TO authenticated
  USING (bucket_id = 'digital-files' AND (storage.foldername(name))[1] = auth.uid()::text);

DROP POLICY IF EXISTS "Users can delete own digital files" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can read own course content" ON storage.objects;
...
DROP POLICY IF EXISTS "Users can delete own course content" ON storage.objects;

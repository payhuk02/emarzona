-- Améliore le fulfillment digital : résoudre user_id via auth.users.email
-- quand le checkout invité n'a pas encore de checkout_user_id.
...
BEGIN
  ...
  -- Invité déjà provisionné (course/service) ou compte existant : lier par email
...
COMMENT ON FUNCTION public.fulfill_digital_order_items_on_paid IS
  'Active/crée licences digitales au paiement ; résout user_id via customer, metadata, ou auth.users.email.';

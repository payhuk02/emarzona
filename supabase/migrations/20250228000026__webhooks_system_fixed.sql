-- =========================================================
-- Migration : Système de Webhooks (Version Corrigée)
-- Date : 28/02/2025
-- Description : Système complet de webhooks pour tous les événements
-- Version corrigée pour gérer les cas où la table existe déjà
-- =========================================================
...
-- Ajouter la colonne is_active si elle n'existe pas (pour les tables existantes)
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.webhooks ADD COLUMN is_active BOOLEAN DEFAULT true;
  END IF;
END $$;
...
-- Créer l'index is_active seulement si la colonne existe
DO $$
BEGIN
  IF EXISTS (
    SELECT FROM information_schema.columns 
...
-- Supprimer toutes les versions existantes de la fonction (y compris celles avec des signatures différentes)
DO $$
BEGIN
  -- Supprimer toutes les versions possibles de trigger_webhook
...
DROP POLICY IF EXISTS "Users can view their webhooks" ON public.webhooks;
DROP POLICY IF EXISTS "Users can create webhooks" ON public.webhooks;
DROP POLICY IF EXISTS "Users can update their webhooks" ON public.webhooks;
DROP POLICY IF EXISTS "Users can delete their webhooks" ON public.webhooks;
...
COMMENT ON FUNCTION trigger_webhook(UUID, TEXT, JSONB) IS 'Déclenche un webhook pour un événement donné';

...
function sanitizeEventForLog(event: SendGridEvent): Record<string, unknown> {
  return {
    event: event.event,
  ...
  try {
    ...
    for (const event of events) {
      try {
        ...
        console.error(`Error processing event ${event.sg_event_id}:`, {
...
async function processSendGridEvent(event: SendGridEvent): Promise<void> {
  ...
  if (campaign_id || custom_args?.campaign_id) {
    ...
    if (actualCampaignId) await updateCampaignMetrics(actualCampaignId, eventType, emailLogId);
  }
  ...
  if (sequence_id || custom_args?.sequence_id) {
    ...
    if (actualSequenceId) await updateSequenceMetrics(actualSequenceId, eventType, emailLogId);
  }

  console.log('Processed SendGrid event', {
    eventType,

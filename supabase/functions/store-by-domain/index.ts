...
serve(async req => {
  ...
  try {
    ...
    const RESERVED_SUBDOMAINS = [
      ...
      'checkout', 'cart', 'order', 'orders', 'cdn', 'assets', 'static', 'media',
      'status', 'health', 'test', 'staging', 'dev', 'docs', 'email', 'mx',
      'ns1', 'ns2', 'dns', 'domain', 'cpanel', 'phpmyadmin', 'wp-admin',
    ];
    if (RESERVED_SUBDOMAINS.includes(subdomain.toLowerCase())) {
      return new Response(
        JSON.stringify({ error: 'Reserved subdomain', message: `The subdomain "${subdomain}" is reserved` }),

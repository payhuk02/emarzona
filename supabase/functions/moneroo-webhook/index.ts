...
serve(async req => {
  ...
  try {
    ...
    if (webhookSecret) {
      ...
      if (isProduction) {
        console.error('MONEROO_WEBHOOK_SECRET not configured in production - rejecting webhook');
        return new Response(
          JSON.stringify({
      ...
      console.warn(
        'MONEROO_WEBHOOK_SECRET not configured (non-production) - webhook signature verification disabled'
      );
    }

    const payload = JSON.parse(rawPayload);
    const safePayloadForLogs = sanitizeWebhookPayload(payload);
    console.log('Moneroo webhook received', safePayloadForLogs);

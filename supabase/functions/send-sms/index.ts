...
function generateSMSMessage(template: string, data: Record<string, unknown>): string {
  ...
  switch (template) {
    case 'payment_success':
      return `✅ Emarzona: Paiement de ${formattedAmount} confirmé${orderNumber ? ` - Commande #${orderNumber}` : ''}. Merci !`;

    case 'payment_failed':
      return `❌ Emarzona: Paiement de ${formattedAmount} échoué${reason ? `. ${reason}` : ''}. Veuillez réessayer.`;

    case 'payment_cancelled':
      return `🚫 Emarzona: Paiement de ${formattedAmount} annulé${reason ? `. ${reason}` : ''}.`;

    case 'payment_refunded':
      return `💸 Emarzona: Remboursement de ${formattedAmount} effectué${transactionId ? ` - Transaction: ${transactionId}` : ''}.`;

    case 'payment_pending':
      return `⏳ Emarzona: Paiement de ${formattedAmount} en attente. Vous recevrez une confirmation prochainement.`;

    default:
      return `Emarzona: ${JSON.stringify(data)}`;
  }
}

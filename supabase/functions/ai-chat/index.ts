 ...
 * Fallback LLM pour le chatbot — utilise la config admin (`ai_management_settings.chatbot`)
 * et appelle Lovable AI Gateway. Réponse non-streamée (JSON simple).
 */
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Charger la config depuis platform_settings via RPC
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({ error: 'Échec de l\'appel IA' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
    }
    ...
    return new Response(
      JSON.stringify({ content, model, latencyMs }),

...
export const useRecommendationTracking = () => {
  ...
  const trackRecommendationClick = useCallback(
    async (data: RecommendationClickData) => {
      if (!user?.id) {
  ...
  const trackRecommendationPurchase = useCallback(
    async (recommendedProductId: string, reason: string) => {
      if (!user?.id) {
      ...
      try {
        ...
        logger.error('Exception tracking recommendation purchase', {
          error,
          recommendedProductId,
          reason,
        });

...
export const useAnalytics = (storeId?: string) => {
  ...
  const trackPageView = useCallback(
    (pageStoreId?: string, additionalData?: Record<string, unknown>) => {
  ...
  const trackAddToCart = useCallback(
    (productId: string, quantity: number = 1, productData?: Record<string, unknown>) => {
      if (!storeId) return;
  ...
  const trackPurchase = useCallback(
    (orderData: { orderId: string; amount: number; currency?: string; items?: unknown[] }) => {
      if (!storeId) return;
  ...
  const trackShare = useCallback(
    (platform: string, contentData?: Record<string, unknown>) => {
      if (!storeId) return;

...
export const usePlatformLogo = () => {
  ...
  // Nettoyer le cache au montage si il contient un ancien logo legacy
  useEffect(() => {
    try {
      ...
      if (cachedLogo) {
        ...
        const hasLegacyReference =
          (cached.light && (cached.light.includes('legacy') || cached.light.includes('/old-logo'))) ||
          (cached.dark && (cached.dark.includes('legacy') || cached.dark.includes('/old-logo')));
  ...
  useEffect(() => {
    ...
    // Nettoyer le cache si il contient un ancien logo legacy
    try {
      const cachedLogo = localStorage.getItem(LOGO_CACHE_KEY);
      if (cachedLogo) {
        const cached = JSON.parse(cachedLogo);
        // Vérifier si le cache contient des références legacy
        const hasLegacyReference =
          (cached.light && (cached.light.includes('legacy') || cached.light.includes('/old-logo'))) ||
          (cached.dark && (cached.dark.includes('legacy') || cached.dark.includes('/old-logo')));
    ...
    const loadFromCache = () => {
      try {
        ...
        if (cachedLogo) {
          ...
          // Vérifier que le cache ne contient pas d'ancien logo legacy
          const hasLegacyReference =
            (cached.light &&
              (cached.light.includes('legacy') || cached.light.includes('/old-logo'))) ||
            (cached.dark && (cached.dark.includes('legacy') || cached.dark.includes('/old-logo')));
          ...
          if (!hasRealData && (cached.light || cached.dark)) {
            ...
            if (cachedUrl && isMounted) {
              // Vérifier que l'URL ne contient pas de marqueurs legacy
              if (cachedUrl.includes('legacy') || cachedUrl.includes('/old-logo')) {
                localStorage.removeItem(LOGO_CACHE_KEY);

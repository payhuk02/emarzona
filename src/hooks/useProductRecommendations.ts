...
export interface ProductRecommendation {
  ...
  recommendation_type: 'similar' | 'category' | 'tags' | 'popular' | 'purchase_history' | 'collaborative' | 'view_based';
}
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (error) {
          ...
          if (errorCode === '42883' || errorMessage.includes('does not exist')) {
            const { data: v1Data, error: v1Error } = await supabase.rpc('get_frequently_bought_together', {
              p_product_id: productId,
              p_limit: limit,
            });

...
const PHYSICAL_WEBHOOK_FIELDS = 'id, store_id, event_type, target_url, is_active, created_at, updated_at';
const PHYSICAL_WEBHOOK_LOG_FIELDS = 'id, webhook_id, event_type, payload, response_status, response_body, error_message, created_at';
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(PHYSICAL_WEBHOOK_FIELDS)
        .eq('store_id', storeId)
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(PHYSICAL_WEBHOOK_LOG_FIELDS)
...
export const useTestWebhook = () => {
  ...
  return useMutation({
    mutationFn: async ({ webhookId, storeId }: { webhookId: string; storeId: string }) => {
      ...
      // Utiliser le système unifié pour les tests
      const { triggerUnifiedWebhook } = await import('@/lib/webhooks/unified-webhook-service');
      await triggerUnifiedWebhook(storeId, webhook.event_type, testPayload, `test-${Date.now()}`);
      return { success: true };
    },
    onSuccess: () => {
      toast({

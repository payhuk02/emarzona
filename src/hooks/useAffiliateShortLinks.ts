...
import {
  ...
  AffiliateError,
  AffiliateErrorCode,
} from '@/lib/affiliate-errors';

const AFFILIATE_SHORT_LINK_FIELDS =
  'id, affiliate_link_id, affiliate_id, short_code, target_url, total_clicks, unique_clicks, custom_alias, expires_at, is_active, created_at, updated_at, last_used_at';
...
export const useAffiliateShortLinks = (affiliateLinkId?: string) => {
  ...
  const createShortLink = useCallback(
    async (formData: CreateShortLinkForm): Promise<AffiliateShortLink | null> => {
      try {
        ...
        if (formData.custom_alias) {
          ...
          if (
            aliasError &&
            aliasError.code !== 'PGRST301' &&
            aliasError.code !== '42P01' &&
            !aliasError.message?.includes('does not exist') &&
            !aliasError.message?.includes('404')
          ) {
        ...
        if (formData.custom_alias) {
          ...
          if (codeError) {
            ...
            logger.warn(
              'RPC function  _unavailable, using client-side code generation:',
              codeError
            );
        ...
        const { data: creationResult, error: creationError } = await supabase.rpc(
          'create_short_link_with_rate_limit',
          {
        ...
        if (creationError) {
          ...
          if (
            creationResult?.error === 'rate_limit_exceeded' ||
            creationError.message?.includes('rate_limit_exceeded') ||
            creationError.details?.reason === 'rate_limit_exceeded'
          ) {
            const remainingTime =
              creationResult?.details?.time_window_minutes ||
              creationError.details?.time_window_minutes ||
              60;
            const maxActions =
              creationResult?.details?.max_actions || creationError.details?.max_actions || 50;
  ...
  const deleteShortLink = useCallback(
    async (shortLinkId: string): Promise<boolean> => {
      try {
   ...
   */
  const toggleShortLink = useCallback(
    async (shortLinkId: string, isActive: boolean): Promise<boolean> => {
      try {
        const { error } = await supabase
          .from('affiliate_short_links')

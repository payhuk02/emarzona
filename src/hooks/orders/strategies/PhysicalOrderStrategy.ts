...
export class PhysicalOrderStrategy implements OrderStrategy {
  async createOrder(context: OrderStrategyContext): Promise<OrderCreationResult> {
    ...
    try {
      ...
      if (invoiceError) {
        logger.error('Error creating invoice for physical order', { error: invoiceError, orderId });
      }
    } catch (invoiceErr) {
      logger.error('Error creating invoice for physical order', { error: invoiceErr, orderId });
    }

...
> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <Card className="m-4 sm:m-8 shadow-sm border border-destructive/20">
          <CardContent className="py-12 text-center space-y-4">
            ...
            <p className="text-muted-foreground max-w-md mx-auto">
              Une erreur est survenue lors de l'affichage de votre boutique.
              {this.state.error?.message && (
                <span className="block text-xs mt-2 font-mono bg-muted p-2 rounded">
                  {this.state.error.message}
                </span>
              )}
            </p>
      ...
      <header ref={headerRef} className="sticky top-0 z-10 border-b bg-card/95 backdrop-blur-sm">
        <div className="flex h-14 sm:h-16 items-center gap-2 sm:gap-4 px-3 sm:px-4 md:px-6">
          ...
          {!loading && stores.length > 0 && (
            <Link
              to={generateStoreUrl(stores[0].slug, stores[0].subdomain)}
              target="_blank"
        ...
        {loading ? (
          ...
          <Card className="shadow-sm border border-destructive/20 bg-card/50 backdrop-blur-sm animate-in fade-in slide-in-from-bottom-4">
            <CardHeader className="text-center py-8 sm:py-12">
              ...
              <CardDescription className="max-w-md mx-auto mt-2">{storesError}</CardDescription>
            </CardHeader>
            <CardContent className="flex justify-center pb-8">
              <Button variant="outline" className="gap-2" onClick={() => void refetchStores()}>
                <RefreshCw className="h-4 w-4" />

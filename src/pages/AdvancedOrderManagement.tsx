...
const AdvancedOrderContent: React.FC<{ store: Store }> = ({ store }) => {
  ...
  const { orders, loading: ordersLoading, refetch: refetchOrders, error: ordersError } = useOrders(store.id, { pageSize: 100 });
  ...
  const handleRefresh = useCallback(async () => {
    try {
      ...
      if (isConnectionError) {
        setConnectionError('Erreur de connexion réseau. Vérifiez votre connexion internet et réessayez.');
        toast({
            ...
            <div>
              <h1 className="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-bold flex items-center gap-1.5 sm:gap-2 mb-1 sm:mb-2">
                <div className="p-1.5 sm:p-2 rounded-lg bg-gradient-to-br from-blue-500/10 to-cyan-500/5 backdrop-blur-sm border border-blue-500/20 animate-in zoom-in duration-500">
                  <CreditCard className="h-4 w-4 sm:h-5 sm:w-5 md:h-6 md:w-6 lg:h-8 lg:w-8 text-blue-500 dark:text-blue-400" aria-hidden="true" />
                </div>
            ...
            <CardHeader className="p-3 sm:p-4 md:p-6">
              <CardTitle className="flex items-center gap-2 text-base sm:text-lg md:text-xl">
                <div className="p-1.5 sm:p-2 rounded-lg bg-gradient-to-br from-blue-500/10 to-cyan-500/5 border border-blue-500/20">
                  <Shield className="h-3.5 w-3.5 sm:h-4 sm:w-4 md:h-5 md:w-5 text-blue-600 dark:text-blue-400" aria-hidden="true" />
                </div>
                    ...
                    >
                      ...
                      <CardContent className="relative p-3 sm:p-4 md:p-5">
                        <div className="flex items-start gap-2 sm:gap-3 mb-2 sm:mb-3">
                          <div className={`p-2 sm:p-2.5 md:p-3 rounded-lg bg-gradient-to-br ${type.bgGradient} border ${type.borderColor} group-hover:scale-110 transition-transform duration-300`}>
                            <Icon className={`h-4 w-4 sm:h-5 sm:w-5 md:h-6 md:w-6 ${type.iconColor}`} aria-hidden="true" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <h3 className="font-semibold text-xs sm:text-sm md:text-base mb-1 sm:mb-1.5">{type.title}</h3>
                            <Badge 
                              variant="secondary" 
              ...
              <Card className="border-border/50 bg-card/50 backdrop-blur-sm">
                <CardHeader className="p-3 sm:p-4 md:p-6">
                  <CardTitle className="flex items-center gap-2 text-base sm:text-lg md:text-xl">
                    <div className="p-1.5 sm:p-2 rounded-lg bg-gradient-to-br from-purple-500/10 to-pink-500/5 border border-purple-500/20">
                      <MessageSquare className="h-3.5 w-3.5 sm:h-4 sm:w-4 md:h-5 md:w-5 text-purple-600 dark:text-purple-400" aria-hidden="true" />
                    </div>
                ...
                <CardContent className="p-3 sm:p-4 md:p-6 pt-0">
                  <div className="mb-3 sm:mb-4 md:mb-6 space-y-2 sm:space-y-3">
                    ...
                    >
                      <SelectTrigger id="order-select" className="w-full h-9 sm:h-10 md:h-11 text-xs sm:text-sm md:text-base min-h-[44px]">
                          ...
                          orders.map((order) => (
                            <SelectItem key={order.id} value={order.id}>
                              <div className="flex items-center justify-between gap-2 w-full">
                                <span className="font-medium text-[10px] sm:text-xs md:text-sm">#{order.order_number}</span>
                  ...
                  {selectedOrderId && store?.id ? (
                    ...
                    <div className="flex flex-col items-center justify-center min-h-[300px] sm:min-h-[400px] text-center p-4 sm:p-6 md:p-12 animate-in fade-in zoom-in-95 duration-500">
                      <div className="p-3 sm:p-4 rounded-full bg-gradient-to-br from-purple-500/10 to-pink-500/5 border border-purple-500/20 mb-3 sm:mb-4 animate-in zoom-in-95 duration-500">
                        <MessageSquare className="h-10 w-10 sm:h-12 sm:w-12 md:h-16 md:w-16 text-purple-600 dark:text-purple-400" aria-hidden="true" />
                      </div>
            ...
            <CardHeader className="p-3 sm:p-4 md:p-6">
              <CardTitle className="flex items-center gap-2 text-base sm:text-lg md:text-xl">
                <div className="p-1.5 sm:p-2 rounded-lg bg-gradient-to-br from-green-500/10 to-emerald-500/5 border border-green-500/20">
                  <Shield className="h-3.5 w-3.5 sm:h-4 sm:w-4 md:h-5 md:w-5 text-green-600 dark:text-green-400" aria-hidden="true" />
                </div>
                    ...
                    >
                      <h3 className="font-semibold text-xs sm:text-sm md:text-base flex items-center gap-1.5 sm:gap-2">
                        <div className={`p-1.5 sm:p-2 rounded-lg bg-gradient-to-br ${feature.bgGradient} border ${feature.borderColor}`}>
                          <Icon className={`h-3.5 w-3.5 sm:h-4 sm:w-4 md:h-5 md:w-5 ${feature.iconColor}`} aria-hidden="true" />
                        </div>
                        {feature.title}
                      </h3>

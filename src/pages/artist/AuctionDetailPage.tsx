...
export default function AuctionDetailPage() {
  ...
  const payAuctionWin = usePayAuctionWin();
  ...
  const isAuctionWinner =
    !!user &&
    auction.status === 'sold' &&
    (winningBid?.bidder_id === user.id || myBids.some(b => b.status === 'winning'));

  const handlePayWinningBid = async () => {
    if (!auction || !user?.email) {
      toast({
        title: 'Connexion requise',
        description: "Connectez-vous avec le compte gagnant pour régler l'enchère.",
        variant: 'destructive',
      });
      return;
    }

    await payAuctionWin.mutateAsync({
      auctionId: auction.id,
    ...
    <ArtistPublicPageShell>
      <SEOMeta
        ...
        description={
          ...
          `Enchère — ${auction.artist_products?.artwork_title ?? auction.auction_title}`
        }
        url={`https://www.emarzona.com/auctions/${auction.auction_slug}`}
      />
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <CardDescription className="mt-2">
                      {auction.artist_products?.artwork_title} par{' '}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-lg font-semibold">
                      {auction.starting_price.toLocaleString('fr-FR')} XOF
                    </p>
                  </div>
                  {auction.reserve_price && (
                    <div>
                      ...
                      <p className="text-lg font-semibold">
                        {auction.reserve_price.toLocaleString('fr-FR')} XOF
                      </p>
                    </div>
                  )}
                  <div>
                    ...
                    <p className="text-lg font-semibold">
                      {auction.minimum_bid_increment.toLocaleString('fr-FR')} XOF
                    </p>
                  </div>
                  {auction.buy_now_price && (
                    <div>
                      ...
                      <p className="text-lg font-semibold">
                        {auction.buy_now_price.toLocaleString('fr-FR')} XOF
                      </p>
                    </div>
                  )}
                </div>
            ...
            {isAuctionWinner && (
              <Card className="border-primary/40 bg-primary/5">
                <CardHeader>
                  ...
                  <CardDescription>
                    Réglez votre enchère pour finaliser l&apos;acquisition de l&apos;œuvre.
                    {auction.winner_payment_deadline && (
                      <>
                        {' '}
                        Délai :{' '}
                        {formatDistanceToNow(new Date(auction.winner_payment_deadline), {
                          addSuffix: true,
                          locale: fr,
                        })}
                      </>
                ...
                <CardContent className="space-y-3">
                  ...
                  <Button
                    ...
                    disabled={payAuctionWin.isPending}
                  >
                    {payAuctionWin.isPending ? (
                      <>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Offre minimum:{' '}
                      {(auction.current_bid + auction.minimum_bid_increment).toLocaleString(
                        'fr-FR'
                      )}{' '}
                      XOF
                    </p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch id="proxy_bid" checked={isProxyBid} onCheckedChange={setIsProxyBid} />
                  </div>
              ...
              <CardContent className="space-y-3 text-sm">
                <div>
                  ...
                  <p className="font-medium">
                    {format(new Date(auction.start_date), 'PPp', { locale: fr })}
                  </p>
                </div>
                <div>
                  <p className="text-muted-foreground">Fin</p>
                  <p className="font-medium">
                    {format(new Date(auction.end_date), 'PPp', { locale: fr })}
                  </p>
                </div>

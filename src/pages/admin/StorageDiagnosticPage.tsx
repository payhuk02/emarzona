...
import { AlertCircle, CheckCircle2, XCircle, Download, Trash2, RefreshCw, FileText, Search, Filter } from 'lucide-react';
...
export default function StorageDiagnosticPage() {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute('download', `storage-diagnostic-${new Date().toISOString().split('T')[0]}.csv`);
            ...
            {report && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Résultats du diagnostic des fichiers storage
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  {report.summary.allExist ? (
                    <Alert>
                      ...
                      <AlertDescription>
                        Tous les fichiers référencés dans la base de données existent dans le bucket.
                      </AlertDescription>
                    </Alert>
                  ) : (
                    <Alert variant="destructive">
                      ...
                      <AlertDescription>
                        {report.missingFiles} fichier(s) référencé(s) en base n'existent pas dans le bucket.
                      </AlertDescription>
                    </Alert>
                  )}
            ...
            {report && report.files.length > 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Liste complète des fichiers vérifiés ({filteredFiles.length} sur {report.files.length})
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {/* Filtres */}
                  <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    <Select value={statusFilter} onValueChange={(value: 'all' | 'exists' | 'missing') => setStatusFilter(value)}>
                          ...
                          {filteredFiles.length === 0 ? (
                            ...
                            <TableRow key={file.attachmentId}>
                              ...
                              <TableCell className="text-xs text-muted-foreground max-w-[300px]">
                                <div className="truncate" title={file.error || file.signedUrlError || ''}>
            ...
            <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    Vous êtes sur le point de supprimer {missingFiles.length} entrée(s) de la base de données
                    pour les fichiers qui n'existent pas dans le bucket.
                    <br />
                    <br />

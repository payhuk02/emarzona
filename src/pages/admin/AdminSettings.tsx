...
const AdminSettings = () => {
  ...
  const readRoutesFromSettings = (settings: unknown): string[] => {
    if (!settings || typeof settings !== 'object') return [];
    const value = (settings as Record<string, unknown>)['require_aal2_routes'];
    return Array.isArray(value) ? value.filter((v): v is string => typeof v === 'string') : [];
  };

  const [routes, setRoutes] = useState<string[]>(readRoutesFromSettings(dbSettings));
  ...
  useEffect(() => {
    setRoutes(readRoutesFromSettings(dbSettings));
  }, [dbSettings]);
    ...
    <AdminLayout>
      <div className="container mx-auto p-6 space-y-6 animate-fade-in">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg sm:text-2xl md:text-3xl font-bold bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">
              Paramètres
            </h1>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Configuration des notifications automatiques</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Activez/Désactivez les capacités par rôle administrateur
                </CardDescription>
              </div>
              ...
              <div className="space-y-6">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Statut actuel d’authentification multi-facteurs
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="flex items-center gap-2">
                      <span>Session administrateur:</span>
                      <Badge variant={isAAL2 ? 'default' : 'destructive'}>
                        {isAAL2 ? 'AAL2 - 2FA active' : 'AAL1 - 2FA inactive'}
                      </Badge>
                    </div>
                  </CardContent>
                </Card>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        for (const r of roles) {
                          const seed = normalizePermissions(
                            roles.find(rr => rr.role === r.role)?.permissions
                          );
                          await updateRolePermissions(r.role, seed);
                        }
                      }}
                    >
                    ...
                    <Button
                      ...
                      onClick={() => {
                        const json = JSON.stringify(
                          roles.map(r => ({ role: r.role, permissions: r.permissions })),
                          null,
                          2
                        );
                        ...
                        a.href = url;
                        a.download = `rbac_${Date.now()}.json`;
                        a.click();
                ...
                {roles
                  .filter(
                    r => !roleSearch || r.role.toLowerCase().includes(roleSearch.toLowerCase())
                  )
                  .map(r => (
                    <div key={r.role} className="border rounded-lg p-4">
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        <Button
                          ...
                          onClick={async () => {
                            const next = Object.fromEntries(
                              Object.keys(r.permissions || {}).map(k => [k, true])
                            );
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            const next = Object.fromEntries(
                              Object.keys(r.permissions || {}).map(k => [k, false])
                            );
                            await updateRolePermissions(r.role, next);
                          }}
                        >
                      ...
                      {(() => {
                        const grouped = DEFAULT_PERMISSION_KEYS.reduce(
                            ...
                            {Object.entries(grouped).map(([grp, perms]) => (
                              <div key={grp}>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                  {perms.map(perm => {
                                    const rolePerms = normalizePermissions(r.permissions);
                                    const defaultPerms = normalizePermissions(
                                      roles.find(rr => rr.role === r.role)?.permissions
                                    );
                                    const checked = Boolean(rolePerms[perm]);
                                    const defaultChecked = Boolean(defaultPerms[perm]);
                                    return (
                                      <label key={perm} className="flex items-center gap-2">
                                        <Checkbox
                                          checked={checked}
                                          onCheckedChange={async v => {
                                            const next = { ...rolePerms, [perm]: Boolean(v) };
                                            await updateRolePermissions(r.role, next);
                                          }}
                                        />
                                        <span
                                          className={`text-sm ${checked !== defaultChecked ? 'text-amber-600' : 'text-muted-foreground'}`}
                                        >
                                          {perm}
        ...
        {can('settings.manage') && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Liste des préfixes de routes admin nécessitant la 2FA
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex flex-wrap gap-2">
                ...
                <Button variant="outline" size="sm" onClick={() => setRoutes(defaultRoutes)}>
                  Réinitialiser par défaut
                </Button>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {routes.length === 0 && (
                  <div className="text-sm text-muted-foreground">Aucune route configurée</div>
                )}
              </div>
                ...
                <Button
                  onClick={async () => {
                    ...
                    await supabase.from('admin_config').upsert(
                      {
                        key: 'admin',
                        settings: { ...baseSettings, require_aal2_routes: routes },

...
const REFERRAL_FIELDS =
  'id, referrer_id, referred_id, referral_code, status, created_at, referrer:profiles!referrals_referrer_id_fkey(display_name, first_name, last_name), referred:profiles!referrals_referred_id_fkey(display_name, first_name, last_name)';
const REFERRAL_COMMISSION_FIELDS =
  'id, referral_id, total_amount, commission_amount, status, created_at';
  ...
  const { data: referralCommissions } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(REFERRAL_COMMISSION_FIELDS)
        .order('created_at', { ascending: false });

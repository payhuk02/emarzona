...
import { exportCommissionsToCSV, exportLinksToCSV } from '@/lib/affiliate-export';
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 sm:gap-4 pt-3 sm:pt-4 border-t">
            ...
            <div className="col-span-2 sm:col-span-1">
              ...
              <Badge
                variant={parseFloat(link.conversionRate) > 2 ? 'default' : 'outline'}
                className="text-xs"
              >
                {link.conversionRate}%
              </Badge>
          ...
          <div className="pt-3 sm:pt-4 border-t">
            <ShortLinkManager affiliateLinkId={link.id} fullUrl={link.full_url} />
          </div>
        </div>
      </CardContent>
    </Card>
...
const AffiliateLinksList = memo(
  ({ links }: { links: AffiliateLink[] }) => {
    return (
      <div className="space-y-3 sm:space-y-4">
...
const AffiliateDashboard = () => {
  ...
  useEffect(() => {
    if (
      commissionsPagination?.page !== undefined &&
      commissionsPagination.page !== commissionsPage
    ) {
  ...
  const linksWithConversionRates = useMemo(
    () =>
      links.map(link => ({
        ...link,
        conversionRate:
          link.total_clicks > 0 ? ((link.total_sales / link.total_clicks) * 100).toFixed(1) : '0',
      })),
    [links]
  );
  ...
  const handleLinksPageChange = useCallback(
    (page: number) => {
      setLinksPage(page);
      goToLinksPage(page);
    },
  ...
  const handleLinksPageSizeChange = useCallback(
    (size: number) => {
  ...
  const handleCommissionsPageChange = useCallback(
    (page: number) => {
  ...
  const handleCommissionsPageSizeChange = useCallback(
    (size: number) => {
  ...
  if (affiliateLoading) {
    return (
      <AppPageShell shellClassName="h-screen">
        ...
        >
          <div>
            ...
            <p className="text-[10px] sm:text-xs md:text-sm lg:text-base text-muted-foreground">
              Bienvenue, {affiliate.display_name || affiliate.email} • Code :{' '}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <CardDescription className="text-xs sm:text-sm">
                        ...
                        {commissionsPagination && commissionsPagination.total > 0 && (
                          <span className="ml-2">
                            ({commissionsPagination.total} commission
                            {commissionsPagination.total > 1 ? 's' : ''})
                          </span>
                        )}
                      </CardDescription>
                    </div>

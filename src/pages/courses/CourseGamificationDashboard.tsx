...
const COURSE_ENROLLMENT_FIELDS =
  'id, course_id, user_id, enrolled_at, progress_percentage, completed_lessons, total_lessons, completion_status, created_at, updated_at';

export default function CourseGamificationDashboard() {
  const { courseId } = useParams<{ courseId: string }>();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState<
    'overview' | 'badges' | 'achievements' | 'leaderboard' | 'history'
  >('overview');
  ...
  const { data: enrollment, isLoading: enrollmentLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(COURSE_ENROLLMENT_FIELDS)
  ...
  const { data: leaderboard = [], isLoading: leaderboardLoading } = useCourseLeaderboard(
    courseId,
    20
  );
    ...
    <AppPageShell>
      <div className="container mx-auto p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
        ...
        >
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">{leaderboard.length} participants</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs
          value={activeTab}
          onValueChange={v =>
            setActiveTab(v as 'overview' | 'badges' | 'achievements' | 'leaderboard' | 'history')
          }
        >
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="text-sm text-muted-foreground">
                  {levelInfo.xpForNextLevel - levelInfo.currentXP} XP nécessaires pour le niveau{' '}
          ...
          <TabsContent value="badges" className="space-y-4 sm:space-y-6">
            <Card>
              <CardHeader>
                <CardTitle>
                  Mes Badges ({badges.length} / {availableBadges.length})
                </CardTitle>
                <CardDescription>Badges obtenus dans ce cours</CardDescription>
              </CardHeader>
              <CardContent>
                {badgesLoading ? (
            ...
            {availableBadges.length > badges.length && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Badges que vous pouvez encore obtenir</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {availableBadges
          ...
          <TabsContent value="achievements" className="space-y-4 sm:space-y-6">
            <Card>
              <CardHeader>
                <CardTitle>
                  Mes Achievements ({achievements.length} / {availableAchievements.length})
                </CardTitle>
                <CardDescription>Achievements obtenus dans ce cours</CardDescription>
              </CardHeader>
              <CardContent>
                {achievementsLoading ? (
          ...
          <TabsContent value="leaderboard" className="space-y-4 sm:space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Top {leaderboard.length} étudiants</CardDescription>
              </CardHeader>
              <CardContent>
                {leaderboardLoading ? (
                  <div className="space-y-3">
          ...
          <TabsContent value="history" className="space-y-4 sm:space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Dernières activités et points gagnés</CardDescription>
              </CardHeader>
              <CardContent>
                {historyLoading ? (
                  <div className="space-y-3">

...
const PaymentMethods = () => {
  ...
  const formatPaymentDetails = (method: SavedStorePaymentMethod) => {
    if (method.payment_method === 'mobile_money') {
      const mobileDetails = method.payment_details as MobileMoneyDetails;
      ...
      const operatorLabel = mobileDetails.operator
        ? getMobileMoneyOperatorsForCountry(mobileDetails.country || 'BF').find(
            op => op.value === mobileDetails.operator
          )?.label || mobileDetails.operator
        : 'N/A';
      return `${mobileDetails.phone}${countryName ? ` (${countryName})` : ''} - ${operatorLabel}`;
    } else if (method.payment_method === 'bank_card') {
      const cardDetails = method.payment_details as BankCardDetails;
      const cardNumber = cardDetails.card_number || '';
      const masked = cardNumber.length > 4 ? `****${cardNumber.slice(-4)}` : '****';
      return `${masked} - ${cardDetails.cardholder_name || 'N/A'}`;
    } else {
      const transferDetails = method.payment_details as BankTransferDetails;
          ...
          <Card className="border-border/50 bg-card/50 backdrop-blur-sm animate-in fade-in zoom-in-95 duration-500">
            <CardContent className="pt-6 sm:pt-8 md:pt-12">
              <div className="text-center py-8 sm:py-12 text-muted-foreground">
                ...
                <p className="text-xs sm:text-sm md:text-base lg:text-lg font-medium mb-1.5 sm:mb-2">
                  Aucune méthode de paiement
                </p>
                <p className="text-[10px] sm:text-xs md:text-sm mb-3 sm:mb-4">
                  Ajoutez vos numéros de mobile money ou cartes bancaires pour faciliter vos
                  retraits
                </p>
                <Button
                  onClick={handleCreate}
              ...
              >
                  ...
                  <div className="flex items-start justify-between gap-2 sm:gap-3">
                    <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <CardTitle className="text-xs sm:text-sm md:text-base lg:text-lg truncate">
                          {method.label}
                        </CardTitle>
                ...
                <CardContent className="p-3 sm:p-4 md:p-6 pt-0">
                  <div className="space-y-2 sm:space-y-3">
                    <div>
                      <p className="text-[9px] sm:text-[10px] md:text-xs text-muted-foreground mb-0.5 sm:mb-1">
                        Détails
                      </p>

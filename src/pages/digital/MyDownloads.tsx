...
export const MyDownloads = () => {
   ...
   */
  const successfulDownloads = filteredDownloads?.filter(
    (d: DownloadWithRelations) => d.download_success
  );
  const failedDownloads = filteredDownloads?.filter(
    (d: DownloadWithRelations) => !d.download_success
  );
    ...
    <AppPageShell mainClassName="overflow-x-hidden">
      <div className="container mx-auto p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-5 md:space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          ...
          <Card>
            ...
            <CardContent className="p-3 sm:p-4 md:p-6 pt-0">
              ...
              <p className="text-[9px] sm:text-[10px] md:text-xs text-muted-foreground">
                {stats.total > 0 ? Math.round((stats.successful / stats.total) * 100) : 0}% de
                succès
              </p>
            </CardContent>
          </Card>
        ...
        <Tabs defaultValue="all" className="w-full">
          <TabsList>
            <TabsTrigger value="all">Tous ({filteredDownloads?.length || 0})</TabsTrigger>
            <TabsTrigger value="successful">
              Réussis ({successfulDownloads?.length || 0})
            </TabsTrigger>
            <TabsTrigger value="failed">Échoués ({failedDownloads?.length || 0})</TabsTrigger>
          </TabsList>
...
const DownloadsList = ({
  ...
  downloads: DownloadWithRelations[];
  loading: boolean;
}) => {
  if (loading) {
    return (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 sm:gap-2.5 md:gap-3 mt-1.5 sm:mt-2 flex-wrap">
                    ...
                    <span className="text-[10px] sm:text-xs md:text-sm text-muted-foreground flex items-center gap-0.5 sm:gap-1">
                      ...
                      {format(new Date(download.download_date), 'dd MMM yyyy à HH:mm', {
                        locale: fr,
                      })}
                    </span>

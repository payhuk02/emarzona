  ...
  const handleDeactivate = useCallback(
    async (activationId: string) => {
      try {
        ...
        logger.error(error instanceof Error ? error : 'Erreur lors de la désactivation', {
          error,
          licenseId,
          activationId,
        });
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xs sm:text-sm md:text-base lg:text-lg">
                    ...
                    <div className="flex items-start gap-3 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 sm:gap-2 mb-0.5 sm:mb-1">
                          ...
                          <Badge
                            variant={activation.is_active ? 'default' : 'secondary'}
                        ...
                        <div className="space-y-0.5 sm:space-y-1 text-[10px] sm:text-xs md:text-sm text-muted-foreground">
                          {activation.os_name && (
                            <p>
                              {activation.os_name} {activation.os_version}
                            </p>
                          )}
                          <p>IP: {activation.ip_address}</p>
                          {activation.country && <p>Pays: {activation.country}</p>}
                          <p className="text-xs">
                            Activé le{' '}
                            {format(new Date(activation.activated_at), 'dd MMM yyyy à HH:mm', {
                              locale: fr,
                            })}
                          </p>
                        </div>

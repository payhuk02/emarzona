...
// Icônes pour les sections par type (disponibles dans l'index)
import { Calendar, GraduationCap } from '@/components/icons';
...
// Lazy loading des sections pour optimiser le chargement initial
const LazyArtGallerySection = lazy(() =>
  import('@/components/marketplace/ArtGallerySection').then(module => ({
    default: module.ArtGallerySection,
  }))
);
import { TypeSpecificSection } from '@/components/marketplace/TypeSpecificSection';
...
const Marketplace = () => {
  ...
  }, [filters, pagination.currentPage, pagination.itemsPerPage, hasSearchQuery, toast]);
  ...
  const marketplaceSeoData = useMemo(
    () => ({
      ...
      keywords:
        'marketplace afrique, produits digitaux, formation en ligne, ebook francophone, templates professionnels, logiciels, services digitaux, paiement mobile money, XOF, FCFA, ecommerce afrique, boutique en ligne, vente en ligne afrique',
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 overflow-x-hidden">
        ...
        {/* Sections dédiées par type - Afficher uniquement si aucun filtre spécifique n'est actif */}
        {filters.productType === 'all' && filters.category === 'all' && filters.search === '' && (
          <>
            {/* Section Produits Digitaux Tendances */}
            <TypeSpecificSection
              productType="digital"
              title="Produits Digitaux Tendances"
              subtitle="Découvrez les produits digitaux les plus populaires : logiciels, ebooks, templates et bien plus"
              icon={Download}
              filterType="trending"
              limit={8}
            />

            {/* Section Nouveaux Produits Physiques */}
            <TypeSpecificSection
              productType="physical"
              title="Nouveaux Produits Physiques"
              subtitle="Les dernières arrivées dans notre catalogue : vêtements, électronique, accessoires et plus encore"
              icon={Package}
              filterType="new"
              limit={8}
            />
            ...
            <TypeSpecificSection
              ...
              subtitle="Les services les plus appréciés par nos clients : consultations, coaching, design et développement"
              icon={Calendar}
              filterType="bestsellers"
              limit={8}
            />

            {/* Section Cours en Ligne Populaires */}
            <TypeSpecificSection
              productType="course"
              title="Cours en Ligne Populaires"
              subtitle="Les formations les plus suivies : développez vos compétences avec nos meilleurs cours"
              icon={GraduationCap}
              filterType="trending"
              limit={8}
            />

            {/* Section Galerie d'Art - Lazy loaded */}
            <Suspense
              fallback={
                <section className="py-8 sm:py-12 lg:py-16 px-3 sm:px-4 bg-slate-900/50">

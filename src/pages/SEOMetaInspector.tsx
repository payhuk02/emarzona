...
import { Search, RefreshCw, Plus, X, CheckCircle2, AlertTriangle, XCircle, ExternalLink } from 'lucide-react';
...
function analyze(r: SeoResult): Issue[] {
  ...
  else if (r.title.length > 60) issues.push({ level: 'warn', label: `Title trop long (${r.title.length})` });
  ...
  else if (r.description.length > 160) issues.push({ level: 'warn', label: `Description trop longue (${r.description.length})` });
  else if (r.description.length < 50) issues.push({ level: 'warn', label: `Description trop courte (${r.description.length})` });
  ...
  if (!r.og['title'] || !r.og['image']) issues.push({ level: 'warn', label: 'Open Graph incomplet' });
  ...
  else if (r.h1.length > 1) issues.push({ level: 'warn', label: `${r.h1.length} H1 (1 recommandé)` });
...
const SEOMetaInspector = () => {
  ...
  const urls = useMemo(() => {
    const main = pathsText
      ...
      .map(p => p.trim())
      .filter(Boolean)
      .map(p => (p.startsWith('http') ? p : `${base.replace(/\/$/, '')}${p.startsWith('/') ? p : `/${p}`}`));
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Vérifiez title, description, canonical, hreflang et balises OG/Twitter pour chaque page publique.
                </p>
              </div>
              <Button
              ...
              <CardContent className="space-y-3">
                <div className="grid gap-3 sm:grid-cols-3">
                  <div className="sm:col-span-1">
                    ...
                    <Input value={base} onChange={e => setBase(e.target.value)} placeholder="https://www.emarzona.com" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="text-xs font-medium text-muted-foreground">Ajouter une page</label>
                ...
                <div className="rounded-lg border border-dashed p-3 space-y-3 bg-muted/20">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Inspecte les pages d'une boutique sur <code>myemarzona.shop</code>. Renseignez le slug pour générer les URLs.
                    </p>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-3">
                    <div className="sm:col-span-1">
                      <label className="text-xs font-medium text-muted-foreground">Slug de la boutique</label>
                      <Input
                        value={storeSlug}
                            ...
                            <div className="space-y-3 pt-2">
                              ...
                              <Tabs defaultValue="basics">
                                ...
                                <TabsContent value="hreflang">
                                  {r.hreflang.length === 0 ? (
                                    <p className="text-sm text-muted-foreground">Aucun hreflang trouvé.</p>
                                  ) : (
                                      ...
                                      <tbody>
                                        {r.hreflang.map((h, i) => (
                                          <tr key={i} className="border-b last:border-0">
                                            ...
                                            <td className="py-2 font-mono text-xs truncate max-w-md">{h.href}</td>
                                          </tr>
                                        ))}
                                      </tbody>
                                    </table>
                                ...
                                <TabsContent value="schema">
                                  {r.jsonLd.length === 0 ? (
                                    <p className="text-sm text-muted-foreground">Aucune donnée structurée détectée.</p>
                                  ) : (
                                    <ScrollArea className="h-64 w-full rounded border bg-muted/30">
                                      <pre className="p-3 text-xs">{JSON.stringify(r.jsonLd, null, 2)}</pre>
                                    </ScrollArea>
                                  )}
                                </TabsContent>
                              </Tabs>
...
function KV({ label, value, link = false }: { label: string; value: string | null; link?: boolean }) {
  return (
    <div className="grid grid-cols-[120px_1fr] gap-3 items-start">
      <div className="text-xs font-medium text-muted-foreground uppercase pt-0.5">{label}</div>
      {value ? (
        link ? (
          <a href={value} target="_blank" rel="noreferrer" className="text-sm break-all hover:underline text-primary">
            {value}
          </a>
        ) : (

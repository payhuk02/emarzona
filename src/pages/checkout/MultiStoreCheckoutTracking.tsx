...
export default function MultiStoreCheckoutTracking() {
  ...
  const getStatusBadge = (status: OrderInfo['status']) => {
    switch (status) {
      ...
      case 'processing':
        return <Badge variant="default" className="bg-blue-600">En traitement</Badge>;
      case 'failed':
  ...
  const pendingCount = ordersWithUrls.filter(o => o.status === 'pending' || o.status === 'processing').length;
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground mt-2">
                  {ordersWithUrls.length} commande{ordersWithUrls.length > 1 ? 's' : ''} créée{ordersWithUrls.length > 1 ? 's' : ''} pour {new Set(ordersWithUrls.map(o => o.storeId)).size} boutique{new Set(ordersWithUrls.map(o => o.storeId)).size > 1 ? 's' : ''}
                </p>
              </div>
            </div>
                      ...
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          <div>
                            ...
                            <div className="font-semibold">{order.amount.toLocaleString('fr-FR')} XOF</div>
                          </div>
                          <div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {pendingCount} commande{pendingCount > 1 ? 's' : ''} nécessite{pendingCount > 1 ? 'nt' : ''} un paiement
                      </p>
                    </div>
                    <Button
                      onClick={() => {
                        const firstPending = ordersWithUrls.find(o => o.status === 'pending' && o.checkoutUrl);

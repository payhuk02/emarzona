...
export default function DemandForecasting() {
  ...
  // State
  const [selectedTab, setSelectedTab] = useState<'forecasts' | 'suggestions' | 'analytics'>(
    'suggestions'
  );
  ...
  const updateSuggestionStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: ReorderSuggestion['status'] }) => {
      const { error } = await supabase.from('reorder_suggestions').update({ status }).eq('id', id);
      ...
      <div className="container mx-auto p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-lg sm:text-2xl font-bold">
                {stats.totalSuggestedQuantity.toLocaleString('fr-FR')}
              </div>
            </CardContent>
          </Card>
        ...
        {stats.critical > 0 && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              <strong>{stats.critical} produit(s) en urgence critique</strong> - Rupture de stock
              imminente. Action requise immédiatement.
            </AlertDescription>
          </Alert>
        )}
            ...
            {suggestions.length === 0 ? (
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>{suggestions.length} suggestion(s) trouvée(s)</CardDescription>
                </CardHeader>
                <CardContent>
                          ...
                          <TableRow key={suggestion.id}>
                            ...
                            <TableCell>
                              <span
                                className={cn(
                                  ...
                                  suggestion.current_stock <= suggestion.safety_stock &&
                            ...
                            <TableCell>
                              {suggestion.estimated_stockout_date
                                ? format(
                                    new Date(suggestion.estimated_stockout_date),
                                    'dd MMM yyyy',
                                    { locale: fr }
                                  )
                                : '-'}
                      ...
                      <CardHeader>
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <CardDescription>
                              {format(new Date(forecast.forecast_period_start), 'dd MMM yyyy', {
                                locale: fr,
                              })}{' '}
                              -{' '}
          ...
          <TabsContent value="analytics" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Analysez la précision de vos prévisions</CardDescription>
              </CardHeader>
              <CardContent>

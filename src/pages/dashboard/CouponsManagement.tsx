...
import { usePromotions, useCreatePromotion, useUpdatePromotion, useDeletePromotion } from '@/hooks/physical/usePromotions';
...
export default function CouponsManagement() {
  ...
  const { data: promotions = [], isLoading: promotionsLoading } = usePromotions({ storeId: store?.id });
  ...
  const [typeFilter, setTypeFilter] = useState<'all' | 'percentage' | 'fixed_amount' | 'buy_x_get_y' | 'free_shipping'>('all');
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      active: promotions.filter((p) => p.is_active && (!p.ends_at || new Date(p.ends_at) > now)).length,
                              ...
                              <div className="text-sm">
                                ...
                                {promotion.ends_at && (
                                  <div className="text-muted-foreground">
                                    jusqu'au {format(new Date(promotion.ends_at), 'dd MMM yyyy', { locale: fr })}
                                  </div>
                                )}
                              </div>
                            </TableCell>
            ...
            <Dialog open={!!viewingUsageId} onOpenChange={(open) => !open && setViewingUsageId(null)}>
              <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Historique des utilisations de ce coupon
                  </DialogDescription>
                </DialogHeader>
                {usagesLoading ? (
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-xs text-muted-foreground">
                                {usage.order_total_before.toLocaleString()} → {usage.order_total_after.toLocaleString()}
                              </div>
                            </div>
                          </div>
                        </CardContent>

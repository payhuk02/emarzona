...
const PRODUCT_LOT_FIELDS =
  'id, physical_product_id, variant_id, warehouse_id, lot_number, batch_number, serial_number, manufacturing_date, expiration_date, best_before_date, initial_quantity, current_quantity, unit_cost, rotation_method, quality_status, bin_location, shelf_location, supplier_batch_number, notes, status, created_at, updated_at, physical_products!inner(id, name, store_id)';
const EXPIRATION_ALERT_FIELDS =
  'id, product_lot_id, alert_type, days_until_expiration, is_active, created_at, updated_at, product_lots!inner(id, lot_number, expiration_date, current_quantity, physical_products!inner(id, name, store_id))';
const LOT_MOVEMENT_FIELDS =
  'id, lot_id, movement_type, quantity, movement_date, reason, reference_type, reference_id, created_at';
  ...
  const { data: alerts = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(EXPIRATION_ALERT_FIELDS)
      ...
      <div className="container mx-auto p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-lg sm:text-2xl font-bold">
                {stats.totalQuantity.toLocaleString('fr-FR')}
              </div>
            </CardContent>
          </Card>
        ...
        {alerts.length > 0 && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              {alerts.length} alerte{alerts.length > 1 ? 's' : ''} d'expiration active
              {alerts.length > 1 ? 's' : ''}. Vérifiez les lots qui expirent bientôt.
            </AlertDescription>
          </Alert>
        )}
            ...
            {filteredLots.length === 0 ? (
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {filteredLots.length} lot{filteredLots.length > 1 ? 's' : ''} trouvé
                        ...
                        {filteredLots.map(lot => {
                          ...
                          return (
                            <TableRow key={lot.id}>
                              ...
                              <TableCell>
                                {(lot as string).physical_products?.name || 'N/A'}
                              </TableCell>
                              <TableCell>
                              ...
                              <TableCell>
                                {lot.expiration_date
                                  ? format(new Date(lot.expiration_date), 'dd MMM yyyy', {
                                      locale: fr,
                                    })
                                  : '-'}
                              </TableCell>
                              <TableCell>
                                {daysUntilExp !== null ? (
                                  ...
                                  >
                                    {daysUntilExp < 0
                                      ? `Expiré (${Math.abs(daysUntilExp)}j)`
                                      : `${daysUntilExp}j`}
                                  </span>
                                ) : (
                                  '-'
            ...
            {alerts.length === 0 ? (
              ...
              <div className="space-y-4">
                {alerts.map(
                  (alert: {
                    id: string;
                    product_lots?: { physical_products?: { name?: string } };
                    alert_type?: string;
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <CardDescription>
                              ...
                              {format(
                                new Date(alert.product_lots?.expiration_date),
                                'dd MMM yyyy',
                                { locale: fr }
                              )}
                            </CardDescription>
                          ...
                          <Badge variant="destructive">
                            {alert.days_until_expiration} jour
                            {alert.days_until_expiration > 1 ? 's' : ''}
                          </Badge>
                        </div>
                      </CardHeader>
                      <CardContent>
              ...
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-muted-foreground">
                    Sélectionnez un lot pour voir ses mouvements
                  </p>
                </CardContent>
              </Card>
            )}
            ...
            <div className="space-y-4 py-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={v => setLotForm({ ...lotForm, rotation_method: v as string })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={v => setLotForm({ ...lotForm, quality_status: v as string })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={
                    !lotForm.physical_product_id || !lotForm.lot_number || createLot.isPending
                  }
                  className="flex-1"
                >
                  {createLot.isPending ? (

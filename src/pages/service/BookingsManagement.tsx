      ...
      bookings.forEach(booking => {
        try {
          const start = parseISO(
            `${booking.booking_date}T${booking.start_time || booking.booking_time || '00:00:00'}`
          );
          const end = parseISO(
            `${booking.booking_date}T${booking.end_time || booking.booking_time || '00:00:00'}`
          );
  ...
  const handleCancelBooking = useCallback(
    async (bookingId: string) => {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Impossible d'annuler la réservation";
  ...
  const handleCompleteBooking = useCallback(
    async (bookingId: string) => {
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Impossible de terminer la réservation';
  ...
  const handleMarkNoShow = useCallback(
    async (bookingId: string) => {
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : 'Impossible de marquer comme absent';

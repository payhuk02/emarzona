...
export default function CartEnhanced() {
  ...
  const { getValue } = usePageCustomization();
  const {
    items,
    summary,
    isLoading,
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl font-bold flex items-center gap-1.5 sm:gap-2">
                    <ShoppingBag className="h-4 w-4 sm:h-5 sm:w-5 md:h-6 md:w-6 lg:h-7 lg:w-7 text-primary" aria-hidden="true" />
                    Mon Panier
                  </h1>
                  <p className="text-xs sm:text-sm md:text-base text-muted-foreground mt-0.5 sm:mt-1 flex items-center gap-2" id="cart-description">
                    <span>{summary.item_count} {summary.item_count > 1 ? 'articles' : 'article'}</span>
            ...
            {summary.discount_amount > 0 && (
              <Alert className="border-green-500 bg-green-50 dark:bg-green-900/20 animate-in fade-in slide-in-from-top-2 duration-500">
                ...
                <AlertDescription className="text-green-800 dark:text-green-200">
                  Félicitations ! Vous économisez {summary.discount_amount.toLocaleString('fr-FR')} XOF grâce à vos codes promo.
                </AlertDescription>
              </Alert>
            )}
      ...
      <AlertDialog open={showClearDialog} onOpenChange={setShowClearDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Êtes-vous sûr de vouloir retirer tous les articles de votre panier ? Cette action est irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

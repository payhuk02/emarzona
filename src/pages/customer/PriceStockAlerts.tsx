...
import {
  ...
  PriceAlert,
  StockAlert,
} from '@/hooks/usePriceStockAlerts';
...
export default function PriceStockAlerts() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        if (userError) {
          logger.error(userError instanceof Error ? userError : 'Error fetching user', {
            error: userError,
          });
  ...
  const stats = useMemo(() => {
    ...
    const priceAlertsReached =
      priceAlerts?.filter(alert => {
        const priceDifference = alert.current_price - alert.target_price;
        return priceDifference <= 0;
      }).length || 0;
    const stockAlertsInStock =
  ...
  const handleDeleteStockAlert = useCallback(
      ...
      <div className="container mx-auto p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
          ...
          <div>
            <h1 className="text-lg sm:text-2xl md:text-3xl lg:text-4xl font-bold flex items-center gap-1.5 sm:gap-2 mb-1 sm:mb-2">
              <div className="p-1.5 sm:p-2 rounded-lg bg-gradient-to-br from-purple-500/10 to-pink-500/5 backdrop-blur-sm border border-purple-500/20 animate-in zoom-in duration-500">
                <Bell
                  className="h-4 w-4 sm:h-5 sm:w-5 md:h-6 md:w-6 lg:h-8 lg:w-8 text-purple-500 dark:text-purple-400 fill-purple-500 dark:fill-purple-400"
                  aria-hidden="true"
                />
              </div>
        ...
        {hasError && (
          ...
          >
            ...
            <AlertDescription>
              {error ||
                'Une erreur est survenue lors du chargement des alertes. Veuillez réessayer.'}
            </AlertDescription>
          </Alert>
        )}
        ...
        <div
          ref={statsRef}
          className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-100"
        >
        ...
        >
          <Tabs
            value={activeTab}
            onValueChange={value => setActiveTab(value as 'price' | 'stock')}
                    ...
                    <div className="space-y-4">
                      <div className="text-center py-8">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Chargement des alertes de prix...
                        </p>
                      </div>
                    </div>
                                  ...
                                  <div className="flex flex-wrap items-center gap-2">
                                    {isTargetReached ? (
                                      <Badge className="bg-green-500 text-white text-xs sm:text-sm">
                                        ...
                                        Objectif atteint ! (
                                        {Math.abs(parseFloat(priceDifferencePercent))}% de
                                        réduction)
                                      </Badge>
                                    ) : (
                                      <Badge variant="outline" className="text-xs sm:text-sm">
                                        Encore {priceDifference.toLocaleString('fr-FR')}{' '}
                                        {alert.currency} à économiser
                                      </Badge>
                                    )}
                                  </div>
                                </div>
                                  ...
                                  <div className="flex flex-wrap items-center gap-2">
                                    {isInStock && stockQuantity != null ? (
                                      <Badge className="bg-green-500 text-white text-xs sm:text-sm">
                                        ...
                                        En stock ! ({stockQuantity} disponible
                                        {stockQuantity > 1 ? 's' : ''})
                                      </Badge>
                                    ) : (
                                      <Badge variant="outline" className="text-xs sm:text-sm">
        ...
        <AlertDialog
          open={deletingPriceAlert !== null}
          onOpenChange={open => !open && setDeletingPriceAlert(null)}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Supprimer l'alerte de prix ?</AlertDialogTitle>
              <AlertDialogDescription>
                Êtes-vous sûr de vouloir supprimer cette alerte de prix ? Vous ne recevrez plus de
                notifications pour ce produit.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        <AlertDialog
          open={deletingStockAlert !== null}
          onOpenChange={open => !open && setDeletingStockAlert(null)}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Supprimer l'alerte de stock ?</AlertDialogTitle>
              <AlertDialogDescription>
                Êtes-vous sûr de vouloir supprimer cette alerte de stock ? Vous ne recevrez plus de
                notifications pour ce produit.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>

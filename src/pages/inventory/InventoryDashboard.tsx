...
export default function InventoryDashboard() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return inventoryItems.filter((item: unknown) => {
      const productName =
        item.physical_product?.product?.name || item.variant?.physical_product?.product?.name || '';
  ...
  useEffect(() => {
    if (itemsError || valueError) {
      const errorMessage =
        itemsError?.message || valueError?.message || "Erreur lors du chargement de l'inventaire";
      ...
      <div className="container mx-auto p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6">
        ...
        >
          {[
            ...
            {
              label: 'Valeur Totale',
              value: `${stats.total_value.toLocaleString('fr-FR')} XOF`,
              icon: DollarSign,
        ...
        {alerts && alerts.length > 0 && (
          <LowStockAlerts
            alerts={alerts}
            className="animate-in fade-in slide-in-from-top-4 duration-500 delay-150"
          />
        )}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-muted/50 backdrop-blur-sm h-auto p-1 w-full grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-1.5 sm:gap-2 sm:inline-flex sm:w-auto">
            <TabsTrigger
              value="csv"
              className="text-[10px] xs:text-xs sm:text-sm min-h-[44px] py-2 sm:py-1.5 md:py-2"
            >

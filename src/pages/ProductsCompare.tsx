...
export default function ProductsCompare() {
  ...
  const [productIds, setProductIds] = useState<string[]>(() => {
    ...
    const saved = localStorage.getItem('products-comparison');
    if (saved) {
      try {
  ...
  const getComparisonFields = (productType: string) => {
    ...
    const typeSpecificFields: Record<string, Array<{ key: string; label: string; type: string }>> = {
      ...
      course: [
        { key: 'total_lessons', label: 'Leçons', type: 'number' },
      ],
      artist: [
        { key: 'artist_type', label: 'Type d\'artiste', type: 'text' },
      ],
    };
  ...
  const getPropertyValue = (product: ComparisonProduct, property: string): string | number | undefined => {
    return (product as Record<string, any>)[property];
  };
  ...
  const comparisonFields = filteredAndSortedProducts.length > 0
              ...
              <div className="space-y-6">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      {filteredAndSortedProducts.length} produit{filteredAndSortedProducts.length > 1 ? 's' : ''} à comparer
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                        ...
                        <thead>
                          <tr className="border-b">
                            ...
                            {filteredAndSortedProducts.map((product) => (
                              <th key={product.id} className="text-center p-3 font-semibold min-w-[200px]">
                        ...
                        <Card key={product.id}>
                          <CardContent className="p-4 space-y-3">
                            ...
                            <div className="text-lg font-bold">
                              {(product.promotional_price || product.price).toLocaleString('fr-FR')} {product.currency}
                            </div>
                            <div className="flex gap-2">
                              <Button
                                size="sm"

...
const StoreAnalytics = ({ storeId }: StoreAnalyticsProps) => {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      const [
        ...
        // Top products only — avoid loading full catalog for charts
        supabase
          .from('products')
      ...
      const previousOrders =
        previousOrdersResult.status === 'fulfilled' && previousOrdersResult.value.data
          ? previousOrdersResult.value.data
          : [];
      const customersCount =
        customersResult.status === 'fulfilled' && customersResult.value.count !== null
          ? customersResult.value.count
          : 0;
      const previousCustomersCount =
        previousCustomersResult.status === 'fulfilled' &&
        previousCustomersResult.value.count !== null
          ? previousCustomersResult.value.count
          : 0;
      const recentOrders =
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.setAttribute(
        'download',
        `analytics-boutique-${new Date().toISOString().split('T')[0]}.csv`
      );
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Suivez les performances de votre boutique</CardDescription>
            </div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      {analytics.ordersGrowth !== 0 ? (
                        <>
                          ...
                          <Badge
                            variant="secondary"
                            className={`text-xs ${analytics.ordersGrowth > 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {analytics.ordersGrowth > 0 ? '+' : ''}
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      {analytics.revenueGrowth !== 0 ? (
                        <>
                          ...
                          <Badge
                            variant="secondary"
                            className={`text-xs ${analytics.revenueGrowth > 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {analytics.revenueGrowth > 0 ? '+' : ''}
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      {analytics.customersGrowth !== 0 ? (
                        <>
                          ...
                          <Badge
                            variant="secondary"
                            className={`text-xs ${analytics.customersGrowth > 0 ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {analytics.customersGrowth > 0 ? '+' : ''}
                            {analytics.customersGrowth.toFixed(1)}%
                          </Badge>
                        </>
                      ) : (
                        <Badge variant="secondary" className="text-xs">
                  ...
                  <div className="space-y-3">
                    {analytics.topProducts.map((product, index) => (
                      ...
                      >
                        ...
                        <Badge variant="secondary">{product.sales_count} ventes</Badge>
                      </div>
                    ))}
                  </div>
                </CardContent>

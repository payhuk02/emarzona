...
describe('PaymentProviderSelector', () => {
  ...
  it('should call onChange when a provider is selected', async () => {
    ...
    const paydunyaOption = screen.getByLabelText(/paydunya/i);
    await user.click(paydunyaOption);

    await waitFor(() => {
      expect(mockOnChange).toHaveBeenCalledWith('paydunya');
    });
  });
      ...
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
          single: vi.fn(() => Promise.resolve({
            data: { payment_provider_preference: 'paydunya' },
            error: null,
          })),
        })),
      })),
  ...
  it('should filter providers based on store settings', async () => {
    ...
    await waitFor(() => {
      ...
      expect(paydunyaCard).toHaveClass('opacity-50');
    });
  });

...
import { ArtistHandlingTimeBadge, ArtistSignatureBadge } from '@/components/products/ArtistInfoBadges';
...
import { ServicePricingTypeBadge, ServiceDepositBadge, ServiceCancellationBadge, ServiceMaxParticipantsBadge } from '@/components/products/ServicePricingBadges';
import { CourseDifficultyBadge, CourseLanguageBadge, CourseDurationBadge, CourseModulesBadge } from '@/components/products/CourseInfoBadges';
...
const UnifiedProductCardComponent: React.FC<UnifiedProductCardProps> = ({
  ...
  const productName = product.type === 'artist' ? (product.artwork_title || product.name) : product.name;
  ...
  const marketplaceBuy = useMarketplaceGuestBuy({
    product: {
      ...
      payment_options: product.type === 'physical' || product.type === 'artist' ? product.payment_options : undefined,
    },
  ...
  const stockStatus = useMemo(() => {
    ...
    if (product.stock === undefined || product.stock === null) {
      return { label: 'Stock limité', color: 'text-yellow-600', icon: AlertTriangle, badge: 'warning' };
    }
    if (product.stock === 0) {
      return { label: 'Rupture de stock', color: 'text-red-600', icon: AlertTriangle, badge: 'destructive' };
    }
    if (product.stock < 10) {
      return { label: `Stock faible (${product.stock})`, color: 'text-orange-600', icon: AlertTriangle, badge: 'warning' };
    }
    return { label: `En stock (${product.stock})`, color: 'text-green-600', icon: CheckCircle2, badge: 'default' };
  }, [product]);
  ...
  const affiliateSettings = useMemo(() => {
    if (product.product_affiliate_settings) {
      if (Array.isArray(product.product_affiliate_settings)) {
        return product.product_affiliate_settings.length > 0 ? product.product_affiliate_settings[0] : null;
      }
  ...
  const hasAffiliate = showAffiliate && affiliateSettings?.affiliate_enabled && (affiliateSettings?.commission_rate || 0) > 0;
      ...
      >
        <Link to={productUrl} className="block w-full h-full">
          ...
          />
            ...
            <div className={cn(
              ...
              product.type === 'artist' && "bg-gradient-to-br from-pink-50 to-purple-50 dark:from-pink-900/20 dark:to-purple-900/20",
              product.type === 'physical' && "bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20",
              product.type === 'service' && "bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20",
              product.type === 'course' && "bg-gradient-to-br from-orange-50 to-amber-50 dark:from-orange-900/20 dark:to-amber-900/20",
              product.type === 'digital' && "bg-muted"
            )}>
              {product.type === 'artist' && <Palette className="h-16 w-16 text-gray-400 opacity-30" />}
              {product.type === 'physical' && <Package className="h-16 w-16 text-gray-400 opacity-30" />}
              {product.type === 'service' && <Calendar className="h-16 w-16 text-gray-400 opacity-30" />}
              {product.type === 'course' && <GraduationCap className="h-16 w-16 text-gray-400 opacity-30" />}
              {product.type === 'digital' && <ShoppingCart className="h-16 w-16 text-gray-400 opacity-20" />}
            </div>
          )}
        </Link>
        ...
        {product.type === 'artist' && (
          <div className="absolute top-2 left-2 z-10 flex flex-wrap gap-2">
            ...
            <ArtistSignatureBadge signatureAuthenticated={product.signature_authenticated} size="sm" />
          </div>
        )}
        ...
        {/* Overlay gradient au hover */}
          ...
          {(isDigital || isArtist || isCourse) && (
            <Button size="sm" asChild>
              <Link to={productUrl}>
                {isArtist ? <Palette className="h-4 w-4 mr-2" /> : <Play className="h-4 w-4 mr-2" />}
                Découvrir
              </Link>
            </Button>
          )}
          ...
          {isDigital && productImage && (
            <Button size="sm" variant="secondary" onClick={handleZoomClick} className="relative z-10">
        ...
        {hasPromotion(product) && priceInfo.discount && (
          <div className="absolute top-1.5 sm:top-2 right-1.5 sm:right-2 z-10">
            <Badge variant="destructive" className="shadow-lg text-[10px] sm:text-xs font-bold px-2 sm:px-2.5 py-0.5 sm:py-1">
              -{priceInfo.discount}%
            </Badge>
          </div>
        )}
          ...
          <DialogContent className="max-w-4xl w-full p-0">
            ...
            {productImage && (
              <div className="relative w-full aspect-[3/2] bg-muted">
                <ResponsiveProductImage src={productImage} alt={productName} sizes="100vw" context="detail" fit="contain" fill={true} />
              </div>
            )}
          </DialogContent>
        </Dialog>
      ...
      <div className="flex-1 flex flex-col p-3 sm:p-4 lg:p-6 gap-2">
        ...
        {product.store && (
          <div className="flex items-center gap-1.5 sm:gap-2 mb-1">
            {product.store.logo_url ? (
              <OptimizedImage src={product.store.logo_url} alt={product.store.name} width={28} height={28} className="w-6 h-6 sm:w-7 sm:h-7 rounded-full object-cover border border-gray-200 dark:border-gray-700 flex-shrink-0" placeholder="empty" />
            ) : (
            ...
            <span className="text-xs sm:text-sm font-semibold text-white truncate">{product.store.name}</span>
        ...
        {product.type === 'artist' && product.artist_name && (
          <div className="flex items-center gap-1.5 sm:gap-2 mb-1">
            ...
            <span className="text-xs sm:text-sm font-semibold text-white">{product.artist_name}</span>
        ...
        {/* Title */}
        <Link to={productUrl}>
          <h3 id={`product-title-${product.id}`} className={cn('font-semibold leading-tight line-clamp-2 text-white hover:text-primary transition-colors', isCompact ? 'text-sm' : 'text-base sm:text-lg')}>
            {productName}
          </h3>
        </Link>
        ...
        <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-2">
          ...
          {product.type === 'artist' ? (
            <>
              <Badge className="bg-pink-600 text-white text-[10px] sm:text-xs px-2 py-0.5">{product.artist_type || 'Artiste'}</Badge>
              {product.edition_type && <Badge variant="secondary" className="bg-purple-600 text-white text-[10px] sm:text-xs px-2 py-0.5">{product.edition_type}</Badge>}
              {product.certificate_of_authenticity && <Badge className="bg-green-600 text-white flex items-center text-[10px] sm:text-xs px-2 py-0.5"><Shield className="h-3 w-3 mr-1" /> Certifié</Badge>}
              {product.artist_type === 'multimedia' && product.video_url && <Badge className="bg-blue-600 text-white flex items-center text-[10px] sm:text-xs px-2 py-0.5"><Video className="h-3 w-3 mr-1" /> Preview vidéo</Badge>}
            </>
          ) : product.type === 'physical' ? (
            <>
              <Badge className="bg-green-600 text-white text-[10px] sm:text-xs px-2 py-0.5">Physique</Badge>
              {stockStatus?.badge === 'destructive' && <Badge variant="destructive" className="text-[10px] sm:text-xs px-2 py-0.5">Rupture</Badge>}
              {stockStatus?.badge === 'warning' && (product.stock || 0) > 0 && <Badge variant="outline" className="bg-orange-500 text-white border-orange-600 text-[10px] sm:text-xs px-2 py-0.5">Stock faible</Badge>}
              {(product as any).free_shipping && <Badge className="bg-blue-600 text-white flex items-center text-[10px] sm:text-xs px-2 py-0.5"><Truck className="h-3 w-3 mr-1" /> Livraison gratuite</Badge>}
              {/* Note: size chart is not part of PhysicalProduct by default, but we will pass it if it exists */}
              {(product as any).size_chart_id && <PhysicalSizeChartBadge sizeChartId={(product as any).size_chart_id} sizeChartName={(product as any).size_chart_name} productSlug={product.slug} storeSlug={product.store?.slug} size="sm" />}
            </>
          ) : product.type === 'service' ? (
            <>
              <Badge className="bg-purple-600 text-white text-[10px] sm:text-xs px-2 py-0.5">{product.service_type || 'Service'}</Badge>
              {product.calendar_available && <Badge className="bg-green-600 text-white flex items-center text-[10px] sm:text-xs px-2 py-0.5"><Calendar className="h-3 w-3 mr-1" /> Calendrier</Badge>}
              {product.booking_required && !product.calendar_available && <Badge variant="secondary" className="bg-blue-600 text-white text-[10px] sm:text-xs px-2 py-0.5">Réservation requise</Badge>}
            </>
          ) : product.type === 'course' ? (
            <>
              <Badge className="bg-orange-600 text-white text-[10px] sm:text-xs px-2 py-0.5">Cours en ligne</Badge>
              {product.access_type && <Badge className="bg-green-600 text-white flex items-center text-[10px] sm:text-xs px-2 py-0.5"><Award className="h-3 w-3 mr-1" /> {product.access_type}</Badge>}
            </>
          ) : (
            <Badge className={cn('text-white text-[10px] sm:text-xs px-2 py-0.5', typeBadge.color)}>{typeBadge.label}</Badge>
          )}
            ...
            {product.rating && product.rating > 0 ? (
              <>
                <div className="flex items-center gap-0.5">
                  {[1, 2, 3, 4, 5].map(star => (
                    <Star key={star} className={cn('h-3 w-3', star <= product.rating! ? 'fill-yellow-400 text-yellow-400' : 'text-gray-400')} />
                  ))}
                </div>
                <span className="text-xs sm:text-sm font-medium text-gray-300">{product.rating.toFixed(1)}</span>
                {!product.hide_reviews_count && product.review_count && <span className="text-xs text-gray-500">({product.review_count})</span>}
              </>
            ) : (
        ...
        <div className="flex flex-wrap gap-2 mb-2 text-xs sm:text-sm">
          ...
          {(product.type === 'digital' || (product.type === 'artist' && product.artist_type === 'multimedia' && product.video_url) || (product.type === 'service' && product.calendar_available) || (product.type === 'course' && product.video_preview)) ? (
             <div className="flex items-center gap-1.5 text-blue-400">
               <Zap className="h-3 w-3" /><span>Instantanée</span>
             </div>
          ) : (product.type === 'artist' || product.type === 'service' || product.type === 'course') ? (
             <div className="flex items-center gap-1.5 text-gray-400">
               <FileText className="h-3 w-3" /><span>En préparation</span>
             </div>
          ) : null}
          ...
          {product.type === 'physical' && (
            <>
              <div className={cn('flex items-center gap-1.5 font-medium', stockStatus?.color)}>
                <StockIcon className="h-3 w-3" /><span>{stockStatus?.label}</span>
              </div>
              {product.weight && (
                <div className="flex items-center gap-1.5 text-gray-400"><Weight className="h-3 w-3" /><span>{product.weight} kg</span></div>
              )}
              {product.shipping_required !== false && (
                <div className="flex items-center gap-1.5 text-gray-400"><MapPin className="h-3 w-3" /><span>Livraison requise</span></div>
              )}
            </>
          )}
          ...
          {product.type === 'service' && (
            <>
              <div className="flex items-center gap-1.5 text-gray-400">
                ...
                <span className="font-medium">{product.duration ? formatDuration(product.duration, product.duration_unit) : 'Sur mesure'}</span>
              </div>
          ...
          {product.type === 'artist' && (
             <>
               {product.artwork_year && <div className="flex items-center gap-1.5 text-gray-400"><Calendar className="h-3 w-3"/><span>{product.artwork_year}</span></div>}
               {product.artwork_medium && <div className="flex items-center gap-1.5 text-gray-400"><Palette className="h-3 w-3"/><span>{product.artwork_medium}</span></div>}
             </>
          )}

          {/* Digital generic key info */}
          {product.type === 'digital' && !isCompact && keyInfo.slice(0, 2).map((info, idx) => (
             <div key={idx} className={cn('flex items-center gap-1.5', info.badge ? 'text-blue-400 font-medium' : 'text-gray-400')}>
               {info.icon && <info.icon className="h-3 w-3" />}<span>{info.value}</span>
             </div>
          ))}

          {/* License */}
          {product.type === 'digital' && product.licensing_type && (
            <Badge variant="outline" className={cn('text-xs', product.licensing_type === 'plr' ? 'border-emerald-500 text-emerald-400' : product.licensing_type === 'copyrighted' ? 'border-red-500 text-red-400' : 'border-blue-500 text-blue-400')}>
              {product.licensing_type === 'plr' ? 'PLR' : product.licensing_type === 'copyrighted' ? "Droit d'auteur" : 'Standard'}
            </Badge>
          )}
        ...
        {product.type === 'artist' && product.edition_type === 'limited_edition' && product.edition_number && product.total_editions && (
           <EditionLimitedBadge productId={product.id} editionNumber={product.edition_number} totalEditions={product.total_editions} />
        )}
        ...
        {product.type === 'service' && (
          <div className="flex flex-wrap gap-2 mb-2">
            ...
            <ServiceDepositBadge depositRequired={(product as any).deposit_required} depositAmount={(product as any).deposit_amount} depositType={(product as any).deposit_type} currency={product.currency || 'XOF'} size="sm" />
            <ServiceCancellationBadge allowCancellation={(product as any).allow_booking_cancellation} cancellationDeadlineHours={(product as any).cancellation_deadline_hours} size="sm" />
            <ServiceMaxParticipantsBadge maxParticipants={(product as any).max_participants} size="sm" />
            </div>
        )}
        ...
        {product.type === 'course' && (
          <div className="flex flex-wrap gap-2 mb-2">
            ...
            <CourseDurationBadge totalDuration={product.total_duration ? Math.round(product.total_duration / 3600) : null} durationUnit="hours" size="sm" />
            {/* lessons_count is inside modules in CourseProduct, or maybe top-level */}
            <CourseModulesBadge modulesCount={product.modules?.length} lessonsCount={(product as any).lessons_count} size="sm" />
          </div>
        )}
        ...
        {product.type === 'physical' && product.variants && product.variants.length > 0 && (
          <div className="mb-2 p-2 bg-blue-900/20 rounded-lg border border-blue-800">
            <span className="text-xs font-medium text-blue-200">{product.variants.length} variations</span>
                    </div>
        )}
        ...
        <div className="mt-auto pt-3 sm:pt-4 border-t border-gray-700">
          ...
          {showActions && (
            <MarketplaceProductCardActions
              ...
              buyLabel={product.type === 'physical' && product.stock === 0 ? 'Épuisé' : (product.type === 'physical' && physicalCheckoutDisplay ? physicalCheckoutDisplay.cta_button_label : marketplaceBuy.cta.buyLabel)}
              ...
              buyIcon={product.type === 'service' && product.calendar_available ? 'calendar' : 'cart'}

...
export const ProductDescriptionTab = ({ formData, updateFormData }: ProductDescriptionTabProps) => {
  ...
  const sanitizeShortDescription = (text: string) => {
    ...
    const cleaned = sanitizeString(text.replace(/<[^>]*>/g, '').replace(/\s+/g, ' '));
  ...
  const analyzeKeywordDensity = (text: string, metaKeywords: string) => {
    ...
    keywords.forEach(keyword => {
      ...
      if (percentage < 0.5) {
        suggestions.push(
          `Augmenter l'utilisation de "${keyword}" (actuellement ${percentage.toFixed(1)}%)`
        );
      } else if (percentage > 3) {
        suggestions.push(
          `Réduire l'utilisation de "${keyword}" (actuellement ${percentage.toFixed(1)}%)`
        );
      }
    });
  ...
  const analyzeContentStructure = (html: string) => {
    ...
    if (headings.length > 0) {
      ...
      if (!hasH2 && headings.length > 1)
        suggestions.push('Utiliser des titres H2 pour structurer le contenu');
  ...
  React.useEffect(() => {
    ...
    // Duplicate content detection
    const duplicates = detectDuplicateContent(
      plain,
      formData.meta_title || '',
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm sm:text-base text-gray-600">
            Créez une description attrayante et optimisez votre référencement
          </p>
        </div>
        <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto flex-wrap">
          ...
          <AIContentGenerator
            productInfo={{
              name: formData.name || '',
              type: formData.product_type || 'digital',
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card id="short_description_section">
            <CardHeader>
              ...
              <CardDescription>Résumé en une phrase pour les aperçus et listes</CardDescription>
            </CardHeader>
            <CardContent>
              <Textarea
              ...
              <div className="mt-2 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                <p
                  ...
                  className={cn(
                    'text-xs',
                    getCounterColorClass((formData.short_description || '').length)
                  )}
                  aria-live="polite"
                >
          ...
          <Card id="description_section">
            <CardHeader>
              ...
              <CardDescription>Description détaillée avec mise en forme riche</CardDescription>
            </CardHeader>
            <CardContent>
              {previewMode ? (
                <SafeHTML
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Liste des points forts et fonctionnalités</CardDescription>
            </CardHeader>
            <CardContent>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="text-center">
                  <div
                    className={cn(
                      'px-2 py-1 inline-block rounded text-sm font-medium',
                      getReadabilityColor(readability)
                    )}
                  >
                ...
                <div className="space-y-2 text-sm">
                  ...
                  >
                    {formData.description && formData.description.length >= 200 ? (
                      <CheckCircle2 className="h-4 w-4 text-green-500" aria-hidden="true" />
                    ) : (
                      <AlertCircle className="h-4 w-4 text-red-500" aria-hidden="true" />
                    )}
                  ...
                  <div className="flex items-center gap-2 px-2 py-1 min-h-[44px]">
                    ...
                    <span>
                      Images avec alt {missingAltCount > 0 ? `(manquants: ${missingAltCount})` : ''}
                    </span>
                  </div>

...
export type LayoutType = 'default' | 'settings' | 'emails' | 'products' | 'orders' | 'customers' | 'analytics' | 'account' | 'sales' | 'finance' | 'marketing' | 'systems' | 'store' | 'bookings' | 'inventory' | 'shipping' | 'promotions' | 'courses' | 'affiliate' | 'digital-portal' | 'physical-portal' | 'minimal';
...
const detectLayoutType = (pathname: string): LayoutType => {
  ...
  if (pathname.includes('/account/digital') || pathname.includes('/my-downloads') || pathname.includes('/my-licenses') || pathname.includes('/digital/updates')) return 'digital-portal';
  if (pathname.includes('/account/physical') || pathname.includes('/physical-inventory') || pathname.includes('/physical-analytics') || pathname.includes('/physical-serial-tracking')) return 'physical-portal';
  if (pathname.includes('/account/courses') || pathname.includes('/dashboard/courses/') || pathname.includes('/dashboard/my-courses')) return 'courses';
  ...
  if (pathname.includes('/bookings') || pathname.includes('/advanced-calendar') || pathname.includes('/service-management') || pathname.includes('/recurring-bookings') || pathname.includes('/services/')) return 'bookings';
  if (pathname.includes('/dashboard/inventory') || pathname.includes('/physical-inventory') || pathname.includes('/physical-lots') || pathname.includes('/physical-serial-tracking') || pathname.includes('/physical-barcode-scanner') || pathname.includes('/physical-preorders') || pathname.includes('/physical-backorders')) return 'inventory';
  if (pathname.includes('/dashboard/shipping') || pathname.includes('/shipping-services') || pathname.includes('/contact-shipping-service') || pathname.includes('/batch-shipping')) return 'shipping';
  
  // 2. Routes avec sidebars existantes
  if (pathname.includes('/settings')) return 'settings';
  if (pathname.includes('/emails')) return 'emails';
  if (pathname.includes('/products') || pathname.includes('/digital-products') || pathname.includes('/bundles') || pathname.includes('/updates')) return 'products';
  if (pathname.includes('/orders') || pathname.includes('/advanced-orders') || pathname.includes('/vendor/messaging')) return 'orders';
  ...
  if (pathname.includes('/product-kits') || pathname.includes('/demand-forecasting') || pathname.includes('/cost-optimization') || pathname.includes('/suppliers') || pathname.includes('/warehouses')) return 'sales';
...
export const MainLayout = ({
  ...
  const hasFixedSidebar = [
    'settings', 'emails', 'products', 'orders', 'customers', 'analytics', 
    'account', 'sales', 'finance', 'marketing', 'systems',

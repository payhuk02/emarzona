...
import { Package, CheckCircle2, Truck, MapPin, Calendar, ExternalLink, Eye } from 'lucide-react';
import {
  useCustomerPhysicalOrders,
  useCustomerPhysicalOrder,
  type CustomerPhysicalOrder,
} from '@/hooks/physical/useCustomerPhysicalOrders';
      ...
      <Card>
        <CardHeader className="pb-3 sm:pb-4 px-3 sm:px-4 pt-3 sm:pt-4">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4">
            ...
            <Badge
              variant="default"
              className="text-sm sm:text-base lg:text-lg px-3 sm:px-4 py-1.5 sm:py-2 self-start sm:self-auto whitespace-nowrap"
            >
          ...
          <CardContent className="space-y-3 sm:space-y-4 px-3 sm:px-4 pb-3 sm:pb-4">
            ...
            {latestShipment.estimated_delivery && (
              <div>
                ...
                <p className="text-xs sm:text-sm text-muted-foreground">
                  {format(new Date(latestShipment.estimated_delivery), 'dd MMMM yyyy', {
                    locale: fr,
                  })}
                </p>
              </div>
            ...
            {latestShipment.actual_delivery && (
              <div>
                ...
                <p className="text-xs sm:text-sm text-muted-foreground">
                  {format(new Date(latestShipment.actual_delivery), 'dd MMMM yyyy à HH:mm', {
                    locale: fr,
                  })}
                </p>
              </div>
          ...
          <CardContent className="px-3 sm:px-4 pb-3 sm:pb-4">
            <div className="space-y-1 text-xs sm:text-sm">
              {(() => {
                ...
                return (
                  <>
                    {address.street && <p className="break-words">{address.street}</p>}
                    <p className="break-words">
                      {address.city}
                      {address.state && `, ${address.state}`}
                      {address.postal_code && ` ${address.postal_code}`}
                    </p>
                    {address.country && <p className="break-words">{address.country}</p>}
                  </>
                );
              })()}
            </div>
        ...
        <CardContent className="px-3 sm:px-4 pb-3 sm:pb-4">
          <div className="space-y-3 sm:space-y-4">
            {order.order_items?.map((item: OrderItem) => (
              <div
                key={item.id}
                className="flex items-start gap-2 sm:gap-4 pb-3 sm:pb-4 border-b last:border-0"
              >
...
export const OrderTracking = ({ orderId: orderIdProp }: OrderTrackingProps) => {
  ...
  if (selectedOrderId) {
    return (
      <div className="space-y-4">
        <Button variant="outline" onClick={() => setSelectedOrderId(null)} className="mb-4">
          ← Retour à la liste
        </Button>

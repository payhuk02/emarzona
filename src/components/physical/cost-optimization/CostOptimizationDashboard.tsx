...
import {
  useProductCosts,
  useMarginAnalysis,
  usePriceOptimizationRecommendations,
  useCalculateProductMargin,
  useGeneratePriceOptimizationRecommendations,
  useUpdateRecommendationStatus,
  type PriceOptimizationRecommendation,
  type MarginAnalysis,
  type ProductCost,
} from '@/hooks/physical/useCostOptimization';
...
export default function CostOptimizationDashboard() {
  ...
  const { data: recommendations, isLoading: recommendationsLoading } =
    usePriceOptimizationRecommendations(store?.id, {
      status: 'pending',
    });
  ...
  const recommendationsWithProducts = (recommendations ||
    []) as PriceOptimizationRecommendationWithProduct[];

  const totalCosts = costsWithProducts.reduce(
    (sum, cost) => sum + (cost.total_cost_per_unit || 0),
    0
  );
  const averageCost = costsWithProducts.length > 0 ? totalCosts / costsWithProducts.length : 0;
  const averageMargin =
    marginsWithProducts.length > 0
      ? marginsWithProducts.reduce((sum, m) => sum + m.net_margin_percentage, 0) /
        marginsWithProducts.length
      : 0;
  const urgentRecommendations = recommendationsWithProducts.filter(r => r.priority === 'urgent');
  const highValueRecommendations = recommendationsWithProducts.filter(
    r => r.expected_profit_change && r.expected_profit_change > 1000
  );
  ...
  return (
    <div className="space-y-3 sm:space-y-4 lg:space-y-6">
      ...
      >
        ...
        >
          <Calculator
            className={`mr-1.5 sm:mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4 ${calculateMargin.isPending ? 'animate-spin' : ''}`}
          />
        ...
        >
          <RefreshCw
            className={`mr-1.5 sm:mr-2 h-3.5 w-3.5 sm:h-4 sm:w-4 ${generateRecommendations.isPending ? 'animate-spin' : ''}`}
          />
            ...
            >
              <CardHeader className="pb-1.5 sm:pb-2 p-2.5 sm:p-3 lg:p-4">
                <CardTitle className="text-[10px] xs:text-xs sm:text-sm font-medium text-muted-foreground flex items-center gap-1 sm:gap-1.5">
                  <div
                    className={`h-5 w-5 sm:h-6 sm:w-6 rounded-lg bg-gradient-to-br ${stat.iconBg} border ${stat.iconBorder} flex items-center justify-center`}
                  >
              ...
              <CardContent className="p-2.5 sm:p-3 lg:p-4 pt-0">
                <div
                  className={`text-lg xs:text-xl sm:text-2xl lg:text-3xl font-bold bg-gradient-to-r ${stat.color} bg-clip-text text-transparent mb-1`}
                >
                  {stat.value}
                </div>
                <p className="text-[10px] xs:text-xs text-muted-foreground">{stat.description}</p>
              </CardContent>
            </Card>
          );
        })}
            ...
            <Card className="border border-border/50 bg-card/50 backdrop-blur-sm hover:shadow-lg transition-all duration-300">
              <CardHeader className="p-3 sm:p-4 lg:p-6">
                ...
                <CardDescription className="text-xs sm:text-sm">
                  {recommendationsWithProducts.length} recommandation
                  {recommendationsWithProducts.length > 1 ? 's' : ''} en attente
                </CardDescription>
              </CardHeader>
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            <TableHead className="text-right text-xs sm:text-sm px-3 sm:px-4">
                              Actions
                            </TableHead>
                          </TableRow>
                        </TableHeader>
                          ...
                          {recommendationsWithProducts.map(rec => (
                            <TableRow key={rec.id} className="hover:bg-muted/50 transition-colors">
                              ...
                              <TableCell className="text-xs sm:text-sm px-3 sm:px-4">
                                <Badge
                                  variant={
                                    rec.recommendation_type === 'increase_price'
                                      ? 'default'
                                      : rec.recommendation_type === 'decrease_price'
                                        ? 'secondary'
                                        : 'outline'
                                  }
                                  className="text-xs"
                              ...
                              <TableCell className="text-xs sm:text-sm px-3 sm:px-4">
                                <div className="flex items-center gap-1">
                                  ...
                                  >
                                    {rec.price_change_percentage > 0 ? '+' : ''}
                                    {rec.price_change_percentage.toFixed(1)}%
                                  </span>
                                </div>
                              </TableCell>
                              <TableCell className="text-xs sm:text-sm px-3 sm:px-4">
                              ...
                              <TableCell className="text-right px-3 sm:px-4">
                                <Select
                                  value={rec.status}
                                  onValueChange={value =>
                                    handleUpdateRecommendationStatus(rec.id, value)
                                  }
                                >
                                  ...
                                  <SelectContent>
                                    ...
                                    <SelectItem value="implemented" className="text-xs sm:text-sm">
                                      Implémenté
                                    </SelectItem>
                                    <SelectItem value="dismissed" className="text-xs sm:text-sm">
                                      Ignoré
                                    </SelectItem>
                                  </SelectContent>
                                </Select>
                          ...
                          <CardContent className="px-3 sm:px-4 pb-4 space-y-3">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-1.5">
                                ...
                                <span
                                  className={`text-sm sm:text-base font-medium ${rec.price_change_percentage > 0 ? 'text-green-600' : rec.price_change_percentage < 0 ? 'text-red-600' : ''}`}
                                >
                                  {rec.price_change_percentage > 0 ? '+' : ''}
                                  {rec.price_change_percentage.toFixed(1)}%
                                </span>
                            ...
                            <div className="flex items-center justify-between pt-2 border-t border-border/50">
                              ...
                              >
                                ...
                                <SelectContent>
                                  ...
                                  <SelectItem value="implemented" className="text-xs">
                                    Implémenté
                                  </SelectItem>
          ...
          >
            <Card className="border border-border/50 bg-card/50 backdrop-blur-sm hover:shadow-lg transition-all duration-300">
              <CardHeader className="p-3 sm:p-4 lg:p-6">
                ...
                <CardDescription className="text-xs sm:text-sm">
                  {marginsWithProducts.length} analyse{marginsWithProducts.length > 1 ? 's' : ''}{' '}
                  disponible{marginsWithProducts.length > 1 ? 's' : ''}
                </CardDescription>
              </CardHeader>
              <CardContent className="p-3 sm:p-4 lg:p-6 pt-0">
                {marginsWithProducts.length === 0 ? (
                        ...
                        <TableBody>
                          {marginsWithProducts.slice(0, 20).map(margin => (
                            ...
                            >
                              ...
                              <TableCell className="text-xs sm:text-sm px-3 sm:px-4">
                                {format(new Date(margin.analysis_period_start), 'dd MMM', {
                                  locale: fr,
                                })}{' '}
                                -{' '}
                                {format(new Date(margin.analysis_period_end), 'dd MMM yyyy', {
                                  locale: fr,
                                })}
                              </TableCell>
                              <TableCell className="text-xs sm:text-sm px-3 sm:px-4">
                              ...
                              <TableCell className="text-xs sm:text-sm px-3 sm:px-4">
                                <span
                                  className={`font-medium ${margin.net_profit >= 0 ? 'text-green-600' : 'text-red-600'}`}
                                >
                                  {new Intl.NumberFormat('fr-FR', {
                                    style: 'currency',
                                    currency: 'XOF',
                              ...
                              <TableCell className="text-xs sm:text-sm px-3 sm:px-4">
                                <Badge
                                  variant={
                                    margin.gross_margin_percentage >= 50
                                      ? 'default'
                                      : margin.gross_margin_percentage >= 30
                                        ? 'secondary'
                                        : 'outline'
                                  }
                                  className="text-xs"
                              ...
                              <TableCell className="text-xs sm:text-sm px-3 sm:px-4">
                                <Badge
                                  variant={
                                    margin.net_margin_percentage >= 20
                                      ? 'default'
                                      : margin.net_margin_percentage >= 10
                                        ? 'secondary'
                                        : 'destructive'
                                  }
                                  className="text-xs"
                        ...
                        >
                          ...
                          <CardContent className="px-3 sm:px-4 pb-4 space-y-3">
                            ...
                            <div className="flex items-center justify-between pt-2 border-t border-border/50">
                              ...
                              <span
                                className={`text-sm sm:text-base font-bold ${margin.net_profit >= 0 ? 'text-green-600' : 'text-red-600'}`}
                              >
                                {new Intl.NumberFormat('fr-FR', {
                                  style: 'currency',
                                  currency: 'XOF',
          ...
          >
            <Card className="border border-border/50 bg-card/50 backdrop-blur-sm hover:shadow-lg transition-all duration-300">
              <CardHeader className="p-3 sm:p-4 lg:p-6">
                ...
                <CardDescription className="text-xs sm:text-sm">
                  {costsWithProducts.length} produit{costsWithProducts.length > 1 ? 's' : ''} avec
                  coûts enregistrés
                </CardDescription>
              </CardHeader>
              <CardContent className="p-3 sm:p-4 lg:p-6 pt-0">
                        ...
                        <TableBody>
                          {costsWithProducts.map(cost => (
                            <TableRow key={cost.id} className="hover:bg-muted/50 transition-colors">
                              ...
                              <TableCell className="text-xs sm:text-sm px-3 sm:px-4">
                                {format(new Date(cost.cost_basis_date), 'dd MMM yyyy', {
                                  locale: fr,
                                })}
                              </TableCell>
                            </TableRow>
                  ...
                  {marginTrendData.length > 0 ? (
                    ...
                    <div className="h-[250px] sm:h-[300px] flex flex-col items-center justify-center text-muted-foreground px-3 sm:px-4">
                      ...
                      <p className="text-xs sm:text-sm text-center max-w-md">
                        Calculez les marges pour voir les graphiques
                      </p>
                    </div>
                  )}
                </CardContent>

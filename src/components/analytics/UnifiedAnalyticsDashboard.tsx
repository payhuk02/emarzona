...
} from '@/hooks/useUnifiedAnalytics';
...
type UnifiedAnalyticsDashboardProps = {
  timeRange?: TimeRange;
...
export const UnifiedAnalyticsDashboard = ({
  ...
  onTimeRangeChange,
  analytics: controlledAnalytics,
  loading: controlledLoading,
  hidePeriodSelect = false,
}: UnifiedAnalyticsDashboardProps) => {
  const [localRange, setLocalRange] = useState<TimeRange>('30d');
  const timeRange = controlledRange ?? localRange;
  const isControlled = controlledAnalytics !== undefined;

  const { analytics: fetchedAnalytics, loading: fetchedLoading } = useUnifiedAnalytics(timeRange, {
    enabled: !isControlled,
  });

  const analytics = controlledAnalytics ?? fetchedAnalytics;
  const loading = controlledLoading ?? fetchedLoading;

  const setTimeRange = (range: TimeRange) => {
    onTimeRangeChange?.(range);
    if (controlledRange === undefined) setLocalRange(range);
  };
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          {!hidePeriodSelect && <Skeleton className="h-10 w-32" />}
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{formatFcfa(analytics.overview.totalRevenue)}</div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Acheteurs (période)</CardTitle>
            <Users className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{analytics.overview.totalCustomers}</div>
            <p className="text-xs text-muted-foreground mt-2">
              Base CRM : {analytics.overview.crmCustomersTotal} · Panier moyen :{' '}
              {formatFcfa(analytics.overview.averageOrderValue)}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Croissance revenu : {analytics.overview.growthRate.toFixed(1)}%
            </p>
          </CardContent>
        </Card>
      </div>
            ...
            {Object.entries(analytics.byProductType).map(([type, data]) => {
              ...
              return (
                <Card key={type}>
                  ...
                  <CardContent>
                    <div className="text-xl font-bold">{formatFcfa(data.revenue)}</div>
                    <p className="text-xs text-muted-foreground mt-1">
                      {data.orders} commandes • {data.units} unités
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Prix moyen: {formatFcfa(data.averagePrice)}
                    </p>
                  </CardContent>
                </Card>
              );
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="font-bold">{formatFcfa(product.revenue)}</p>
              ...
              <div className="space-y-3">
                {analytics.topCustomers.map((customer, index) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="font-bold">{formatFcfa(customer.totalSpent)}</p>
                      <p className="text-xs text-muted-foreground">
                        {customer.orders} commandes • Moy: {formatFcfa(customer.averageOrderValue)}
                      </p>
                    </div>
                  </div>
                ))}
                ...
                {analytics.revenueOverTime.slice(-30).map(day => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <span className="text-sm font-medium">{formatFcfa(day.revenue)}</span>
                      <span className="text-xs text-muted-foreground">{day.orders} commandes</span>
                    </div>
                  </div>
                ))}
              </div>

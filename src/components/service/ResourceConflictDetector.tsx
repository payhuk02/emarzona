...
const RESOURCE_CONFLICT_FIELDS = 'id, store_id, conflict_type, booking_ids, conflict_date, conflict_start_time, conflict_end_time, staff_member_ids, resource_ids, status, resolution_method, resolved_by, resolved_at, suggested_resolutions, metadata';
const SERVICE_BOOKING_FIELDS = 'id, product_id, user_id, staff_member_id, scheduled_date, scheduled_start_time, scheduled_end_time, status, customer_notes';
  ...
  const { data: conflicts = [], isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(RESOURCE_CONFLICT_FIELDS)
        .eq('store_id', storeId)
        .eq('status', 'detected')

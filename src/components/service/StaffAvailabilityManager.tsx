...
const SERVICE_STAFF_MEMBER_FIELDS = 'id, store_id, name, is_active';
const STAFF_TIME_OFF_FIELDS = 'id, store_id, staff_member_id, start_date, end_date, start_time, end_time, time_off_type, status, reason, auto_block_bookings';
const STAFF_CUSTOM_HOURS_FIELDS = 'id, store_id, staff_member_id, specific_date, day_of_week, start_time, end_time, is_override, is_unavailable, valid_from, valid_until, is_active, created_at';
const STAFF_WORKLOAD_ALERT_FIELDS = 'id, store_id, staff_member_id, alert_date, alert_period_start, alert_period_end, current_bookings_count, max_recommended_bookings, booking_density_percentage, alert_level, is_resolved, suggested_actions';
  ...
  const { data: staffMembers = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(SERVICE_STAFF_MEMBER_FIELDS)
        .eq('store_id', storeId)
        .eq('is_active', true)
  ...
  const { data: customHours = [] } = useQuery({
    ...
    queryFn: async () => {
      let  query= supabase
        ...
        .select(STAFF_CUSTOM_HOURS_FIELDS)
        .eq('store_id', storeId)
  ...
  const { data: workloadAlerts = [] } = useQuery({
    ...
    queryFn: async () => {
      let  query= supabase
        ...
        .select(STAFF_WORKLOAD_ALERT_FIELDS)
        .eq('store_id', storeId)

...
export const NotificationAnalyticsDashboard = () => {
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      const { data: channelLogs, error: channelError } = await supabase
      ...
      setChannelStats(computedChannelStats);
      ...
      parsedTypeLogs.forEach((log) => {
        ...
        const currentStats = typeMap.get(type);
        if (!currentStats) {
          return;
        }
        ...
        if (['delivered', 'opened', 'clicked'].includes(log.status ?? '')) {
          currentStats.delivered += 1;
        }
        if (['opened', 'clicked'].includes(log.status ?? '')) {
          currentStats.opened += 1;
        }
      });

      const computedTypeStats: TypeStats[] = Array.from(typeMap.entries())
      ...
      setTypeStats(computedTypeStats);
    } catch (error) {

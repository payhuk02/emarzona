...
export interface SEOMetaProps {
  ...
  /** Si fourni, remplace les hreflang fr par défaut (ex. landing multilingue) */
...
export const SEOMeta = ({
  ...
  const safeDescription = safeString(
    description,
    'Emarzona - Plateforme de ecommerce et marketing'
  );
  const truncatedDescription =
    safeDescription.length > 160 ? safeDescription.substring(0, 157) + '...' : safeDescription;
  ...
  return (
    <Helmet>
      ...
      {hreflangAlternates && hreflangAlternates.length > 0 ? (
        hreflangAlternates.map(alt => (
          <link
            ...
            href={safeString(alt.href, window.location.origin)}
          />
        ))
      ) : (
        <>
          <link rel="alternate" hrefLang="fr-FR" href={safeString(url, window.location.origin)} />
          <link rel="alternate" hrefLang="fr" href={safeString(url, window.location.origin)} />
          <link
            rel="alternate"
            hrefLang="x-default"
            href={safeString(url, window.location.origin)}
          />
        </>
      )}
      ...
      <meta
        property="og:image"
        content={safeString(image, 'https://www.emarzona.com/og-image.png')}
      />
      ...
      <meta
        ...
        content={safeString(image, '').endsWith('.png') ? 'image/png' : 'image/jpeg'}
      />

      {/* Article Meta (si type = article) */}
      {type === 'article' && publishedTime ? (
        <meta property="article:published_time" content={safeString(publishedTime)} />
      ) : null}
      {type === 'article' && modifiedTime ? (
        <meta property="article:modified_time" content={safeString(modifiedTime)} />
      ) : null}
      {type === 'article' && author ? (
        <meta property="article:author" content={safeString(author)} />
      ) : null}
      {type === 'article' && section ? (
        <meta property="article:section" content={safeString(section)} />
      ) : null}
      {type === 'article' && tags
        ? tags.map(tag => (
      ...
      {type === 'product' && price !== undefined && price !== null && currency ? (
        <meta property="product:price:amount" content={safeString(price)} />
      ) : null}
      {type === 'product' && price !== undefined && price !== null && currency ? (
        <meta property="product:price:currency" content={safeString(currency, 'XOF')} />
      ) : null}
      {type === 'product' && price !== undefined && price !== null && currency && availability ? (
        <meta property="product:availability" content={safeString(availability)} />
      ) : null}
      {type === 'product' && price !== undefined && price !== null && currency ? (

...
export function PortfolioComments({ portfolioId, className }: PortfolioCommentsProps) {
  ...
  const getAuthorName = (comment: PortfolioComment) => {
    if (comment.user) {
      return comment.user.user_metadata?.full_name || comment.user.email?.split('@')[0] || 'Utilisateur';
    }
            ...
            <div className="space-y-6">
              {comments.map((comment) => (
                <div key={comment.id} className="space-y-4">
                  ...
                  <div className={cn(
                    'flex gap-4',
                    comment.is_pinned && 'bg-purple-50 dark:bg-purple-950/20 p-4 rounded-lg border border-purple-200 dark:border-purple-800'
                  )}>
                    <Avatar>
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="flex items-center gap-4">
                        <Button
                          ...
                          className={cn(
                            comment.is_liked && 'text-red-600 hover:text-red-700'
                          )}
                        >
                          <Heart className={cn('h-4 w-4 mr-1', comment.is_liked && 'fill-current')} />
                          {comment.likes_count}
                        </Button>
                        <Button
                          variant="ghost"
                          ...
                          {comment.replies.map((reply) => (
                            <div key={reply.id} className="flex gap-3">
                              ...
                              <div className="flex-1 space-y-1">
                                ...
                                >
                                  <Heart className={cn('h-3 w-3 mr-1', reply.is_liked && 'fill-current text-red-600')} />
                                  {reply.likes_count}
                                </Button>
                              </div>
                            </div>

...
export async function generateInvoicePDF(invoice: Invoice): Promise<Blob> {
  ...
  doc.text(formatCurrency(invoice.subtotal, invoice.currency), pageWidth - margin, yPosition, {
    align: 'right',
  });
  yPosition += 7;
  ...
  if (invoice.tax_amount > 0) {
    if (invoice.tax_breakdown && invoice.tax_breakdown.length > 0) {
      invoice.tax_breakdown.forEach(tax => {
        ...
        doc.text(formatCurrency(tax.amount, invoice.currency), pageWidth - margin, yPosition, {
          align: 'right',
        });
        yPosition += 7;
      });
  ...
  doc.text(formatCurrency(invoice.total_amount, invoice.currency), pageWidth - margin, yPosition, {
    align: 'right',
  });

...
export async function getUserBookingNotificationPreferences(
  ...
  const { data, error } = await supabase
    ...
    .select(
      'email_enabled,sms_enabled,push_enabled,in_app_enabled,reminder_24h_enabled,reminder_2h_enabled,reminder_30min_enabled,confirmation_enabled,reminder_enabled,cancellation_enabled,reschedule_enabled,completion_enabled'
    )
    .eq('user_id', userId)
    .maybeSingle();

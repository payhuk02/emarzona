...
export const getCommissionPaymentHistory = async (
  ...
  try {
    ...
    const { data: payments, error } = await supabase
      .from(tableName)
      .select(
        'id,affiliate_id,referrer_id,commission_ids,amount,currency,payment_method,payment_details,status,approved_at,approved_by,processed_at,transaction_reference,notes,created_at,updated_at'
      )

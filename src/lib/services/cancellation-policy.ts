...
const SERVICE_CANCELLATION_REFUND_FIELDS =
  'id,booking_id,policy_id,original_amount,refund_percentage,refund_amount,cancellation_fee,net_refund_amount,refund_method,hours_before_service,cancellation_reason,is_emergency,emergency_reason,original_order_id,original_payment_id,refund_transaction_id,status,requested_at,processed_at,completed_at,admin_notes,customer_notes,metadata,created_at,updated_at';
...
export async function getServiceCancellationPolicy(
  ...
  const { data, error } = await supabase
    ...
    .select(SERVICE_CANCELLATION_POLICY_FIELDS)
...
export async function processServiceRefund(refundId: string): Promise<ServiceCancellationRefund> {
  ...
  const { data: refund, error: refundError } = await supabase
    .from('service_cancellation_refunds')
    .select(SERVICE_CANCELLATION_REFUND_FIELDS)
    .eq('id', refundId)
    .single();

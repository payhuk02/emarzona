...
class RateLimiter {
  ...
  canMakeRequest(identifier: string = 'global'): boolean {
    const key = this.config.identifier ? `${this.config.identifier}:${identifier}` : identifier;
...
/**
 * Rate Limiter global pour GeniusPay
 * Limite: 100 requêtes par minute par défaut
 */
export const geniuspayRateLimiter = new RateLimiter({
  maxRequests: parseInt(import.meta.env.VITE_GENIUSPAY_RATE_LIMIT_MAX || '100', 10),
  windowMs: parseInt(import.meta.env.VITE_GENIUSPAY_RATE_LIMIT_WINDOW_MS || '60000', 10), // 1 minute
});
...
export function createUserRateLimiter(userId: string): RateLimiter {
  return new RateLimiter({
    maxRequests: parseInt(import.meta.env.VITE_GENIUSPAY_RATE_LIMIT_USER_MAX || '50', 10),
    windowMs: parseInt(import.meta.env.VITE_GENIUSPAY_RATE_LIMIT_WINDOW_MS || '60000', 10),
    identifier: 'user',
  });
}
...
export function createStoreRateLimiter(storeId: string): RateLimiter {
  return new RateLimiter({
    maxRequests: parseInt(import.meta.env.VITE_GENIUSPAY_RATE_LIMIT_STORE_MAX || '200', 10),
    windowMs: parseInt(import.meta.env.VITE_GENIUSPAY_RATE_LIMIT_WINDOW_MS || '60000', 10),
    identifier: 'store',
  });
}
...
export function checkRateLimit(identifier?: string): void {
  const canMakeRequest = identifier
    ? geniuspayRateLimiter.canMakeRequest(identifier)
    : geniuspayRateLimiter.canMakeRequest();

  if (!canMakeRequest) {
    const timeUntilReset = identifier
      ? geniuspayRateLimiter.getTimeUntilReset(identifier)
      : geniuspayRateLimiter.getTimeUntilReset();

...
export async function autoGenerateArtistCertificate(
  ...
  try {
    ...
    const { data: artistProduct, error: artistError } = await supabase
      ...
      .select(
        'id,artist_name,artwork_title,artwork_medium,artwork_year,edition_number,total_editions,signature_authenticated,certificate_of_authenticity,artwork_edition_type'
      )

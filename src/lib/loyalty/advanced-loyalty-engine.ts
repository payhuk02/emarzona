...
export class AdvancedLoyaltyEngine {
  ...
  private static readonly USER_LOYALTY_PROFILE_FIELDS =
    'id,user_id,total_points,available_points,spent_points,current_tier_id,referral_code,preferences,created_at,updated_at';
  private static readonly LOYALTY_RULE_FIELDS =
    'id,name,description,event_type,points,multiplier,conditions,is_active,priority,store_id,valid_from,valid_until,created_at,updated_at';
  private static readonly LOYALTY_TIER_FIELDS =
    'id,name,description,level,min_points,max_points,benefits,badge_color,badge_icon,store_id,created_at,updated_at';
  ...
  async getUserProfile(userId: string): Promise<UserLoyaltyProfile> {
    try {
      ...
      const { data: profile, error } = await supabase
        ...
        .select(AdvancedLoyaltyEngine.USER_LOYALTY_PROFILE_FIELDS)
        .eq('user_id', userId)
        .single();
  ...
  private async getApplicableRules(eventType: LoyaltyRule['eventType'], storeId?: string): Promise<LoyaltyRule[]> {
    ...
    let query = supabase
      .from('loyalty_rules')
      .select(AdvancedLoyaltyEngine.LOYALTY_RULE_FIELDS)
  ...
  private async getCurrentTier(userId: string, totalPoints: number): Promise<LoyaltyTier> {
    ...
    if (!tiers) {
      const { data, error } = await supabase
        .from('loyalty_tiers')
        .select(AdvancedLoyaltyEngine.LOYALTY_TIER_FIELDS)

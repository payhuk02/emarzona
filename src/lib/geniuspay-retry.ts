/**
 * Système de retry avec backoff exponentiel pour GeniusPay
 */
...
import { RetryOptions } from './geniuspay-types';
  ...
  while (attempt <= maxRetries) {
    try {
      ...
      if (!isRetryableError(error)) {
        logger.warn(`[GeniusPayRetry] Non-retryable error, stopping retries:`, {
      ...
      if (attempt >= maxRetries) {
        logger.error(`[GeniusPayRetry] Max retries (${maxRetries}) reached, giving up:`, {
          error: error instanceof Error ? error.message : String(error),
          attempts: attempt + 1,
        });
        throw error;

...
export const validatePromotionData = (data: {
  ...
  if (data.min_purchase_amount !== undefined && data.min_purchase_amount !== null) {
    const minAmount =
      typeof data.min_purchase_amount === 'string'
...
export const checkCodeUniqueness = async (
  ...
  storeId: string,
  excludeId?: string,
  tableName: 'promotions' | 'product_promotions' = 'product_promotions'
...
export const getErrorMessage = (error: unknown): string => {
  const pgError = error && typeof error === 'object' ? (error as PostgresLikeError) : undefined;

  if (pgError?.code === '23505') {
    if (pgError.message?.includes('code')) {
  ...
  if (pgError?.code === '23503') {
    if (pgError.message?.includes('store_id')) {
  ...
  return pgError?.message || 'Une erreur est survenue lors de la création de la promotion';
};

...
const PRODUCT_TEMPLATE_FIELDS =
  'id,name,description,product_type,category,icon,preview_image,template_data,created_by,is_public,usage_count,created_at,updated_at';
...
export async function getProductTemplates(
  ...
  try {
    ...
    logger.error('Error fetching product templates', { error: errorMessage });
...
export async function createProductFromTemplate(
  ...
  try {
    ...
    const errorMessage = caughtError instanceof Error ? caughtError.message : 'Unknown error';
    logger.error('Error creating product from template', { error: errorMessage, templateId });
...
export async function saveProductTemplate(
  ...
  try {
    ...
    const errorMessage = caughtError instanceof Error ? caughtError.message : 'Unknown error';
    logger.error('Error saving product template', { error: errorMessage });
    return { success: false, error: errorMessage };
  }
}

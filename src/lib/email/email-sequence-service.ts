...
const EMAIL_SEQUENCE_FIELDS =
  'id,store_id,name,description,trigger_type,trigger_config,status,enrolled_count,completed_count,created_by,created_at,updated_at';
...
export class EmailSequenceService {
  ...
  static async getSteps(sequenceId: string): Promise<EmailSequenceStep[]> {
    try {
      const { data, error } = await supabase
        .from('email_sequence_steps')
        .select(EMAIL_SEQUENCE_STEP_FIELDS)
  ...
  /**
   * Inscrire un contact par email (résout user_id via RPC)
   */
  static async enrollByEmail(
    sequenceId: string,
    storeId: string,
    email: string,

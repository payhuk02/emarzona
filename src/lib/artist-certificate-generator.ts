 ...
 */
export async function generateArtistCertificatePDF(data: ArtistCertificateData): Promise<Blob> {
  try {
    ...
    doc.setFont('helvetica', 'normal');
    const introText =
      "Le présent certificat atteste que l'œuvre décrite ci-dessous est authentique et provient directement de l'artiste.";
    ...
    if (data.signedByArtist) {
      ...
      if (data.signedDate) {
        yPosition += 6;
        doc.text(
          `Date de signature: ${format(new Date(data.signedDate), 'dd MMMM yyyy', { locale: fr })}`,
          margin + 15,
          yPosition
        );
      }
    ...
    const footerText =
      'Ce certificat est un document officiel. En cas de doute, contactez le vendeur.';

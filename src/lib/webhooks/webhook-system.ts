...
export async function getWebhooks(storeId: string): Promise<Webhook[]> {
  const { data, error } = await supabase
    .from('webhooks')
    .select(
      'id,store_id,url,secret,description,events,status,last_triggered_at,failed_deliveries,created_at,updated_at'
    )
    .eq('store_id', storeId)
...
export async function getWebhookLogs(webhookId: string, limit: number = 50): Promise<WebhookLog[]> {
  const { data, error } = await supabase
    .from('webhook_deliveries')
    .select(
      'id,webhook_id,event_type,event_data,status,response_status_code,response_body,error_message,attempt_number,max_attempts,triggered_at,delivered_at,failed_at'
    )
    .eq('webhook_id', webhookId)
    .order('triggered_at', { ascending: false })
    .limit(limit);

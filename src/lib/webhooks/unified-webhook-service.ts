...
export async function triggerUnifiedWebhook(
  ...
  try {
    ...
    const finalEventId =
      eventId || `${normalizedEventType}-${Date.now()}-${Math.random().toString(36).substring(7)}`;
    ...
    const webhookResults = data as unknown as Array<{
      webhook_id: string;
      delivery_id: string;
    }> | null;
...
export async function triggerWebhooks(
  ...
  logger.warn('triggerWebhooks() is deprecated. Use triggerUnifiedWebhook() instead.', {
    storeId,
    eventType,
  });

  ...
  private async handleGeneralInquiry(
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('ai-chat', {
        body: { messages: [...history, { role: 'user', content: userMessage }] },
      });
      if (!error && data?.content) {
        return { message: data.content as string };
      }
      if (error) {
        logger.warn('ai-chat fallback failed, using static response', { error });
      }
    } catch (e) {
      logger.warn('ai-chat fallback exception', { error: e });
    }

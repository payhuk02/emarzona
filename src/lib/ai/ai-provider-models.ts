...
export const GOOGLE_FREE_TEXT_MODEL_IDS = new Set<string>(GOOGLE_FREE_TEXT_MODELS.map(m => m.id));

export function isFreeGoogleModel(modelId: string): boolean {
  return GOOGLE_FREE_TEXT_MODEL_IDS.has(modelId);
}
...
export function isFreeAiModel(modelId: string, provider: AiGatewayProvider): boolean {
  if (provider === 'google') return isFreeGoogleModel(modelId);

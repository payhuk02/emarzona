 ...
 *
 * Env (first match wins for proxy URL):
 ...
 *
 * Fixie load-balances across TWO outbound IPs. MoneyFusion rejects any IP not
 * on the API KEY allowlist. Pin to one IP with:
 *   FIXIE_OUTBOUND_IP / MONEYFUSION_EGRESS_IP = x.x.x.x
 * (rewrites proxy hostname to that IP — Fixie-supported single-IP mode).
 *
...
/** Optional single outbound IP (Fixie has 2; MF often whitelists only one). */
export function getPinnedEgressIp() {
  const pin = (process.env.FIXIE_OUTBOUND_IP || process.env.MONEYFUSION_EGRESS_IP || '').trim();
  return IPV4_RE.test(pin) ? pin : '';
}

/**
 * Proxy URL used for CONNECT. When FIXIE_OUTBOUND_IP is set, hostname is replaced
 * with that IP so every request exits via the same address.
 */
export function getStaticEgressProxyUrl() {
  const raw = rawProxyUrl();
  if (!raw) return '';

  const pin = getPinnedEgressIp();
  if (!pin) return raw;

  try {
    const u = new URL(raw);
    if (u.hostname === pin) return raw;
    u.hostname = pin;
    return u.toString();
  } catch {
    return raw;
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on('end', () => {
      ...
      resolve({
        ...
        pinnedEgressIp: getPinnedEgressIp() || null,
      });
    });
    socket.on('error', err => {
      clearTimeout(timer);

...
const TEST_CONFIG = {
  // Utilisateur vendeur
  vendorEmail: 'vendor-test@emarzona.com',
  vendorPassword: 'TestPassword123!',

  // Utilisateur acheteur
  buyerEmail: 'buyer-test@emarzona.com',
...
test.describe('Workflow Produit Physique Complet', () => {
  ...
  test('2. Configurer variantes (tailles et couleurs)', async ({ page }) => {
    ...
    for (const variant of TEST_CONFIG.variants) {
      ...
      if (variant.priceAdjustment > 0) {
        await page.fill(
          'input[placeholder*="Ajustement prix"]',
  ...
  test('6. Acheter produit physique avec adresse de livraison', async ({ page }) => {
    ...
    await page.fill(
      ...
      TEST_CONFIG.purchaseQuantity.toString()
    );
  ...
  test.afterAll(async ({ browser }) => {
    ...
    if (await productCard.isVisible()) {
      ...
      await expect(page.locator(`text=${TEST_CONFIG.productName}`)).not.toBeVisible({
        timeout: 5000,
      });
    }
...
test.describe('Validations produits physiques', () => {
  test('Empêcher achat si stock insuffisant', async ({ page }) => {
    ...
    await expect(page.locator('text=/stock insuffisant|out of stock/i')).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test('Validation adresse de livraison requise', async ({ page }) => {
    ...
    await expect(page.locator('text=/adresse requise|address required/i')).toBeVisible();
  });
  ...
  test('SKU unique par produit', async ({ page }) => {
    ...
    await expect(page.locator('text=/SKU existe déjà|SKU already exists/i')).toBeVisible();
  });
});

...
test.describe('Workflow Produit Digital Complet', () => {
  ...
  test('7. Vérifier licence générée après achat', async ({ page, context }) => {
    ...
    await expect(
      page.locator('text=/[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}/')
    ).toBeVisible();
  });
...
test.describe("Validations et cas d'erreur", () => {
  test('Empêcher achat sans connexion', async ({ page }) => {
    ...
    await expect(page.locator('text=/connexion|login|authentification/i')).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test('Empêcher téléchargement sans achat', async ({ page }) => {
    ...
    await expect(page.locator('text=/accès refusé|non autorisé|access denied/i')).toBeVisible();
  });

...
const TEST_CONFIG = {
  ...
  // Utilisateur client
  clientEmail: 'client-test@emarzona.com',
  ...
  staffEmail: 'staff-test@emarzona.com',
...
test.describe('Workflow Service Complet', () => {
  ...
  test('6. Réserver un créneau pour le service', async ({ page }) => {
    ...
    await page.fill(
      'input[name="numberOfParticipants"]',
      TEST_CONFIG.numberOfParticipants.toString()
    );
  ...
  test.afterAll(async ({ browser }) => {
    ...
    if (await serviceCard.isVisible()) {
      ...
      await expect(page.locator(`text=${TEST_CONFIG.serviceName}`)).not.toBeVisible({
        timeout: 5000,
      });
    }
...
test.describe('Validations services', () => {
  test('Empêcher réservation sur créneau déjà pris', async ({ page }) => {
    ...
    await expect(page.locator('text=/créneau non disponible|slot unavailable/i')).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test('Validation nombre de participants max', async ({ page }) => {
    ...
    await expect(page.locator('text=/nombre maximum|max participants/i')).toBeVisible();
  });

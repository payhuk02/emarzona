/**
 * @deprecated Utiliser tests/e2e/cart-checkout-workflow.spec.ts (playwright testDir: tests/e2e).
 * Conservé pour référence ; exécution : npx playwright test tests/cart-checkout.spec.ts --config=playwright.config.ts
 */
...
test.describe('Panier', () => {
  test('devrait pouvoir ajouter un produit au panier', async ({ page }) => {
    ...
    if (await firstProduct.isVisible()) {
      ...
      const addToCartButton = page
        .locator('button:has-text("Ajouter"), button:has-text("Panier"), button:has-text("Cart")')
        .first();
      ...
      if (await addToCartButton.isVisible()) {
        ...
        const cartCounter = page
          .locator('[data-testid="cart-count"], .cart-badge, text=/\\d+/')
          .first();
  ...
  test('devrait afficher le panier', async ({ page }) => {
    ...
    const cartButton = page
      .locator(
        '[data-testid="cart-button"], button:has-text("Panier"), button:has-text("Cart"), [aria-label*="cart"]'
      )
      .first();
    ...
    if (await cartButton.isVisible()) {
      ...
      const cartContent = page.locator(
        '[data-testid="cart-content"], [role="dialog"], .cart-drawer'
      );
  ...
  test('devrait persister le panier après refresh', async ({ page, context }) => {
    ...
    if (await firstProduct.isVisible()) {
      ...
      const addToCartButton = page
        .locator('button:has-text("Ajouter"), button:has-text("Panier")')
        .first();
  ...
  test('devrait pouvoir modifier la quantité', async ({ page }) => {
    ...
    const cartButton = page
      .locator('[data-testid="cart-button"], button:has-text("Panier")')
      .first();
    ...
    if (await cartButton.isVisible()) {
      ...
      const decrementButton = page
        .locator('button:has-text("-"), button[aria-label*="decrease"]')
        .first();
      ...
      if ((await incrementButton.count()) > 0) {
        ...
        const initialQty =
          (await quantityDisplay.textContent()) || (await quantityDisplay.inputValue());
  ...
  test('devrait pouvoir supprimer un produit', async ({ page }) => {
    ...
    if (await cartButton.isVisible()) {
      ...
      const removeButton = page
        .locator(
          'button:has-text("Supprimer"), button:has-text("Remove"), button[aria-label*="remove"]'
        )
        .first();
...
test.describe('Checkout', () => {
  test('devrait accéder à la page de checkout', async ({ page }) => {
    ...
    if (await cartButton.isVisible()) {
      ...
      const checkoutButton = page
        .locator(
          'button:has-text("Commander"), button:has-text("Checkout"), button:has-text("Payer")'
        )
        .first();
  ...
  test('devrait avoir un bouton de confirmation', async ({ page }) => {
    ...
    const confirmButton = page
      .locator('button:has-text("Confirmer"), button:has-text("Payer"), button[type="submit"]')
      .first();

...
test.describe('Service Products', () => {
  ...
  test('should view service calendar', async ({ page }) => {
    ...
    // Should see calendar
    await expect(page.locator('.rbc-calendar, [data-testid="calendar"]')).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test('should create service booking', async ({ page }) => {
    ...
    if (await firstService.isVisible()) {
      ...
      if (await bookButton.isVisible()) {
        ...
        // Should show success
        await expect(page.locator('text=/réservation confirmée|booking confirmed/i')).toBeVisible({
          timeout: 5000,
        });
      }
    }
  ...
  test('should cancel booking', async ({ page }) => {
    ...
    if (await firstBooking.isVisible()) {
      // Click cancel button
      const cancelButton = firstBooking.locator(

...
test.describe('Physical Products', () => {
  ...
  test('should filter low stock products', async ({ page }) => {
    ...
    const lowStockButton = page.locator(
      'button:has-text("Stock faible"), button:has-text("Low Stock")'
    );
  ...
  test('should update stock quantity', async ({ page }) => {
    ...
    const updateButton = firstProduct.locator(
      'button:has-text("Modifier"), button:has-text("Update")'
    );
    if (await updateButton.isVisible()) {
      ...
      await expect(page.locator('text=/succès|success|mis à jour/i')).toBeVisible({
        timeout: 5000,
      });
    }
  });
  ...
  test('should create product variant', async ({ page }) => {
    ...
    // Add variant
    const addVariantButton = page.locator(
      'button:has-text("Ajouter variant"), button:has-text("Add variant")'
    );

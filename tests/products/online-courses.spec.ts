...
test.describe('Online Courses', () => {
  ...
  test('should create online course', async ({ page }) => {
    ...
    // Should redirect to courses list or dashboard
    await expect(page).toHaveURL(/\/(dashboard\/courses|courses)/, { timeout: 10000 });
  ...
  test('should enroll in course', async ({ page }) => {
    ...
    if (await enrollButton.isVisible()) {
      ...
      await expect(page.locator('text=/paiement|payment|inscription/i')).toBeVisible({
        timeout: 5000,
      });
    }
  });
  ...
  test('should watch course lesson', async ({ page }) => {
    ...
    if (await firstCourse.isVisible()) {
      ...
      if (await firstLesson.isVisible()) {
        ...
        await expect(page.locator('video, iframe, [data-testid="lesson-content"]')).toBeVisible({
          timeout: 5000,
        });
      }
    }
    ...
    if (await firstCourse.isVisible()) {
      ...
      if (await quizButton.isVisible()) {
        ...
        if (await firstQuestion.isVisible()) {
          ...
          // Should show results
          await expect(page.locator('text=/résultat|score|result/i')).toBeVisible({
            timeout: 5000,
          });
        }
      }
  ...
  test('should download certificate', async ({ page }) => {
    ...
    if (await completedCourse.isVisible()) {
      ...
      const certificateButton = page.locator(
        'button:has-text("Certificat"), button:has-text("Certificate")'
      );

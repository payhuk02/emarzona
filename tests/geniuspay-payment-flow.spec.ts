/**
 * Tests E2E pour le flux de paiement GeniusPay
 * 
 * Pour exécuter: npm run test:e2e geniuspay-payment-flow
 */

import { test, expect } from '@playwright/test';

test.describe('Flux de Paiement GeniusPay', () => {
  ...
  test('Devrait remplir et soumettre le formulaire de checkout', async ({ page }) => {
    ...
    // Intercepter la requête vers GeniusPay
    await page.route('**/functions/v1/geniuspay', async (route) => {
      // Simuler une réponse GeniusPay réussie
      await route.fulfill({
        status: 200,
        ...
        body: JSON.stringify({
          ...
          data: {
            id: 'test-payment-id',
            checkout_url: 'https://checkout.geniuspay.io/test-payment-id',
          },
        }),
      });
    });
    ...
    // Vérifier que l'utilisateur est redirigé vers GeniusPay
  ...
  test('Devrait valider les montants selon les limites GeniusPay', async ({ page }) => {
    // Aller au checkout
    await page.goto('/checkout?productId=test-product-id&storeId=test-store-id');
...
test.describe('Performance et Optimisation', () => {
  test('Devrait charger le module GeniusPay de manière lazy', async ({ page }) => {
    ...
    // Vérifier que le bundle GeniusPay n'est pas chargé initialement
    const response = await page.goto('/');
    const scripts = await page.locator('script[src*="geniuspay"]').count();
    
    // Le script GeniusPay ne devrait pas être présent dans le HTML initial
    expect(scripts).toBe(0);
    
    // Aller au checkout (devrait charger GeniusPay)
    await page.goto('/checkout?productId=test-product-id&storeId=test-store-id');
    await page.waitForLoadState('networkidle');
    
    // Vérifier que GeniusPay est chargé maintenant
    const geniuspayLoaded = await page.evaluate(() => {
      return typeof window !== 'undefined' && 
             (window as any).__GENIUSPAY_LOADED__ === true;
    });
    
    // Note: Cela nécessite d'ajouter un flag dans geniuspay-lazy.ts
